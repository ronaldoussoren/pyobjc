...
def main():
    asset = AVFoundation.AVAsset.assetWithURL_(
        Foundation.NSURL.fileURLWithPath_isDirectory_(MEDIAPATH, False)
    )
    playerItem = AVFoundation.AVPlayerItem.playerItemWithAsset_(asset)

    callbacks = (
        MediaToolbox.kMTAudioProcessingTapCallbacksVersion_0,
        42,
        init,
        finalize,
        prepare,
    ...
    Foundation.NSRunLoop.currentRunLoop().runUntilDate_(
        Foundation.NSDate.dateWithTimeIntervalSinceNow_(5)
    )
