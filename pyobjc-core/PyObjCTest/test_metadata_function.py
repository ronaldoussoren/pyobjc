...
_FunctionTable = [
    ...
    ("makeIntArrayOf5", b"^i", "", dict(retval=dict(c_array_of_fixed_length=5))),
    ("makeStringArray", b"^*", "", dict(retval=dict(c_array_delimited_by_null=True))),
    ("makeIntArrayOf_", b"^ii", "", dict(retval=dict(c_array_length_in_arg=0))),
    ("nullIntArrayOf5", b"^i", "", dict(retval=dict(c_array_of_fixed_length=5))),
    ("nullStringArray", b"^*", "", dict(retval=dict(c_array_delimited_by_null=True))),
    ("nullIntArrayOf_", b"^ii", "", dict(retval=dict(c_array_length_in_arg=0))),
    (
        "sumX_andY_",
        b"i^i^i",
        "",
    ...
    (
        ...
        b"ii^i",
        "",
        dict(arguments={1: dict(type_modifier=objc._C_OUT, null_accepted=False)}),
    ),
    (
        "swapX_andY_",
        b"v^d^d",
...
class TestArraysOut(TestCase):
    ...
    def testWithCountInResult(self):
        self.maxDiff = None
        self.assertEqual(
            fillArray_uptoCount_.__metadata__(),
            {
