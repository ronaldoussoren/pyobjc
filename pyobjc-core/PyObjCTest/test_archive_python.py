...
if sys.version_info[:2] >= (3, 15):
    ...
    class frozendict_subclass(frozendict):  # noqa: F821
        pass
...
class TestKeyedArchiveSimple(TestCase):
    ...
    def test_simple_frozendict(self):
        if not self.isKeyed:
            # Frozendict is unarchived as dict with a non-keyed archiver
            return
        o = frozendict({1: 2})  # noqa: F821
        p = {o: 9}

        buf = self.archiverClass.archivedDataWithRootObject_(o)
        self.assertIsInstance(buf, NSData)
        v = self.unarchiverClass.unarchiveObjectWithData_(buf)
        self.assertIsInstance(v, frozendict)  # noqa: F821
        self.assertEqual(v, o)
    ...
    @min_python_release("3.15")
    def testSimpleFrozenDictSubclass(self):
        o = frozendict_subclass({})
        o.a = 1
        buf = self.archiverClass.archivedDataWithRootObject_(o)
        self.assertIsInstance(buf, NSData)
        v = self.unarchiverClass.unarchiveObjectWithData_(buf)
        self.assertIsInstance(v, frozendict_subclass)
        self.assertEqual(v, o)
        self.assertEqual(v.a, o.a)

        o = frozendict_subclass({"hello": "bar", 42: 1.5})
        o.a = 99
        buf = self.archiverClass.archivedDataWithRootObject_(o)
        self.assertIsInstance(buf, NSData)
        v = self.unarchiverClass.unarchiveObjectWithData_(buf)
...
class TestKeyedArchivePlainPython(TestCase, test.pickletester.AbstractPickleTests):
    ...
    def dumps(self, arg, proto=0, fast=0, buffer_callback=None):
        # Ignore proto and fast
        self.assertIs(buffer_callback, None)
...
class TestArchivePlainPython(TestKeyedArchivePlainPython):
    ...
    def test_recursive_frozendict_subclass_in_key(self):
        # This fails with NSArchiver beause frozendict is read back as a dict
        # (NSArchiver is deprecated by Apple, don't use it)
        pass
