...
class bdist_mpkg (Command):
    ...
    def finalize_options (self):
        ...
        if tools.is_framework_python() and sys.version_info < (2, 4, 3):
            # Universal Python does not recommend this
    ...
    def finalize_package_data(self):
        ...
        self.packagesdir = os.path.join(self.get_metapackage(), self.component_directory)
    ...
    def run_subproject(self, scheme, setupfile):
        ...
        pkg = self.sub_setup(setupfile, args).get_command_obj('bdist_mpkg').metapackagename
        if pkg is not None:
    ...
    def run(self):
        ...
        for scheme in self.get_schemes():
            ...
            if not os.path.exists(schemedir):
                # if the scheme dir doesn't already exist, nothing was installed there
                # installation should be done in run_extra or by the install command
