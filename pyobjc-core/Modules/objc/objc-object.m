                                      ...
                                      PyObject* _Nullable args, PyObject* _Nullable kwds)
{
    ...
    if (cobject != NULL) {
        ...
        if (PyLong_Check(attrval)) {
            ...
            if (unlikely(p == NULL && PyErr_Occurred())) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                Py_DECREF(attrval);
                return NULL;
                // LCOV_EXCL_STOP
...
static PyObject* _Nullable object_repr(PyObject* _self)
{
    ...
    if (unlikely(flags & PyObjCObject_kMAGIC_COOKIE)) { // LCOV_BR_EXCL_LINE
        // LCOV_EXCL_START
        //
...
object_verify_type(PyObject* obj)
{
    ...
    if (PyObjCObject_GetFlags(obj) & PyObjCObject_kMAGIC_COOKIE) {
        ...
        if (tp != Py_TYPE(obj)) {
            ...
            Py_XDECREF(tmp);
            if (did_change) {
                if (unlikely(PyObjCClass_CheckMethodList((PyObject*)tp, 0)
                             < 0)) { // LCOV_BR_EXCL_LINE
                    // LCOV_EXCL_START
                    Py_DECREF(tp);
                    return -1;
                                               ...
                                               const char* name_bytes)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (PyObjCClass_Check(base)) {
            ...
            if (unlikely(PyObjCClass_CheckMethodList(base, 0) < 0)) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                Py_CLEAR(mro);
                return NULL;
                // LCOV_EXCL_STOP
        ...
        if (PyObjCClass_Check(base)) {
            ...
            if (!hidden) {
                ...
                if (descr) {
                    ...
                    /* TryResolve... can only fail due to internal errors */
                    // LCOV_EXCL_START
                    Py_CLEAR(mro);
                    return NULL;
                    // LCOV_EXCL_STOP
...
static inline PyObject* _Nullable _type_lookup_harder(PyTypeObject* tp, PyObject* name)
{
    ...
    for (i = 0; i < n; i++) { // LCOV_BR_EXCL_LINE
        Class        cls;
        Method*      methods;
        unsigned int method_count, j;
        base = PyTuple_GET_ITEM(mro, i);

        if (unlikely(!PyObjCClass_Check(base))) { // LCOV_BR_EXCL_LINE
        ...
        if (cls == NULL) {
            /* Reached the root of the ObjC class hierarchy, parent
             * classes don't have selectors in the ObjC runtime.
             */
            descr = NULL;
            break;
        }
...
create_object_dict(PyObject* obj __attribute__((__unused__)), PyObject** pdict)
{
    ...
    Py_BEGIN_CRITICAL_SECTION(obj);
    tmp = *(PyObject * PyObjC_ATOMIC*)pdict;
    if (tmp == NULL) {
        *(PyObject * PyObjC_ATOMIC*)pdict = tmp = PyDict_New();
    }
    Py_END_CRITICAL_SECTION();
...
static PyObject* _Nullable object_getattro(PyObject* obj, PyObject* name)
{
    ...
    if (unlikely(!PyUnicode_Check(name))) { // LCOV_BR_EXCL_LINE
    ...
    if (unlikely(namestr == NULL)) { // LCOV_BR_EXCL_LINE
        return NULL;                 // LCOV_EXCL_LINE
    }

    if (unlikely(object_verify_type(obj) == -1)) { // LCOV_BR_EXCL_LINE
    ...
    if (descr == NULL) {
        ...
        if (unlikely(descr == NULL && PyErr_Occurred())) { // LCOV_BR_EXCL_LINE
            /* _type_lookup will only raise due to internal errors */
            return NULL; // LCOV_EXCL_LINE
        }
    }
    ...
    if (descr != NULL) {
        ...
        if (f != NULL && Py_TYPE(descr)->tp_descr_set != NULL) {
            ...
            if (unlikely(res == NULL && !PyErr_Occurred())) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                Py_CLEAR(descr);
    ...
    if (dictptr != NULL) {
        ...
        if (PyObjC_is_ascii_string(name, "__dict__")) {
            if (unlikely(create_object_dict( // LCOV_BR_EXCL_LINE
                             obj, dictptr)
                         == -1)) {
                // LCOV_EXCL_START
                res = NULL;
            ...
            dict = *(PyObject * PyObjC_ATOMIC*)dictptr;
            if (dict != NULL) {
    ...
    if (descr == NULL) {
        ...
        if (unlikely(descr == NULL && PyErr_Occurred())) { // LCOV_BR_EXCL_LINE
            /* _type_lookup_harder only raises on internal errors */
            return NULL; // LCOV_EXCL_LINE
        }
    }
...
object_setattro(PyObject* obj, PyObject* name, PyObject* _Nullable value)
{
    ...
    if (((PyObjCClassObject*)tp)->useKVO) {
        if (!PyObjC_is_ascii_prefix(name, "_", 1)) {
            ...
            if (unlikely(obj_name == nil)) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                PyErr_SetString(PyObjCExc_Error,
    ...
    descr = _type_lookup(tp, name, namestr);
    if (unlikely(descr == NULL && PyErr_Occurred())) { // LCOV_BR_EXCL_LINE
        /* _type_lookup only raises due to internal errors */
        return -1; // LCOV_EXCL_LINE
    }
    f = NULL;
    if (descr != NULL) {
    ...
    if (dictptr != NULL) {
        ...
        if (dict == NULL && value != NULL) {
            /* We're setting a value, ensure the dict exists */
            if (unlikely(create_object_dict( // LCOV_BR_EXCL_LINE
                             obj, dictptr)
                         == -1)) {
                // LCOV_EXCL_START
        ...
        if (dict != NULL) {
            ...
            if (unlikely(
                    res < 0
                    && PyErr_ExceptionMatches(PyExc_KeyError))) { // LCOV_BR_EXCL_LINE
...
{
    if (unlikely(object_verify_type(self) == -1)) { // LCOV_BR_EXCL_LINE
        return NULL;                                // LCOV_EXCL_LINE
    }

    return Py_NewRef((PyObject*)Py_TYPE(self));
}
        ...
        if (v != NULL) {
            ...
            if (typestr != NULL) {
                ...
                if (unlikely(v == NULL)) { // LCOV_BR_EXCL_LINE
                    /* This can only happen with invalid ObjC runtime information */
                    return NULL; // LCOV_EXCL_LINE
                }
...
static PyObject* _Nullable meth_dir(PyObject* self)
{
    ...
    while (cls != NULL) {
        ...
        for (i = 0; i < method_count; i++) {
            ...
            if (unlikely(sel == NULL)) // LCOV_BR_EXCL_LINE
                continue;              // LCOV_EXCL_LINE
            ...
            if (unlikely(PySet_Add(result, item) == -1)) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                free(methods);
                Py_DECREF(result);
                Py_DECREF(item);
...
PyObject* _Nullable _PyObjCObject_NewDeallocHelper(id objc_object)
{
    ...
    if (unlikely(cls_type == NULL)) { // LCOV_BR_EXCL_LINE
        return NULL;                  // LCOV_EXCL_LINE
    }
    ...
    if (unlikely(PyObjCClass_CheckMethodList( // LCOV_BR_EXCL_LINE
                     (PyObject*)Py_TYPE(res), 1)
                 < 0)) {
        /* It is close to impossible to trigger an error here in testing */
        // LCOV_EXCL_START
        Py_DECREF(res);
        return NULL;
        // LCOV_EXCL_STOP
...
PyObject* _Nullable PyObjCObject_GetAttrString(PyObject* obj, char* name)
{
    ...
    if (unlikely(pyname == NULL)) // LCOV_BR_EXCL_LINE
