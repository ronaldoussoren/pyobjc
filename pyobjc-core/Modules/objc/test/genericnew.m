...
static int mod_exec_module(PyObject* m)
{
    ...
    if (PyModule_AddObject(m, // LCOV_BR_EXCL_LINE
                "OC_GenericNewChild", PyObjC_IdToPython([OC_GenericNewChild class]))
        < 0) {
        return -1; // LCOV_EXCL_LINE
    }
    if (PyModule_AddObject(m, // LCOV_BR_EXCL_LINE
                "OC_GenericNewChild2", PyObjC_IdToPython([OC_GenericNewChild2 class]))
        < 0) {
        return -1; // LCOV_EXCL_LINE
    }
    if (PyModule_AddObject(m, // LCOV_BR_EXCL_LINE
                "OC_GenericNewChild3", PyObjC_IdToPython([OC_GenericNewChild3 class]))
        < 0) {
        return -1; // LCOV_EXCL_LINE
    }
    return 0;
