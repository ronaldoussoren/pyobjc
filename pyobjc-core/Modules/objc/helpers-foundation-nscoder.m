                                        ...
                                        __attribute__((__unused__)))
{
    ...
    void (^block)(id, const char*, const void*) =
        ^(id _Nullable self, const char* typestr, const void* buf) {
          ...
          if (unlikely(v1 == NULL)) // LCOV_BR_EXCL_LINE
              goto error;           // LCOV_EXCL_LINE

          v2 = pythonify_c_value(typestr, buf);
          if (unlikely(v2 == NULL)) // LCOV_BR_EXCL_LINE
              goto error;           // LCOV_EXCL_LINE
                                        ...
                                        __attribute__((__unused__)))
{
    ...
    void (^block)(id, const char*, void*) =
        ^(id _Nullable self, const char* typestr, void* buf) {
          ...
          Py_DECREF(result);
          if (unlikely(err == -1)) // LCOV_BR_EXCL_LINE
              goto error;          // LCOV_EXCL_LINE
                                             ...
                                             __attribute__((__unused__)))
{
    ...
    void (^block)(id, const char*, void*, NSUInteger) = ^(
      ...
      if (unlikely(actual_size == -1)) { // LCOV_BR_EXCL_LINE
          // LCOV_EXCL_START
          PyObjCErr_ToObjCWithGILState(&state);
          return;
          // LCOV_EXCL_STOP
    ...
    PyObject* method, PyObject* self, PyObject* const* arguments, size_t nargs)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (unlikely(PyTuple_GET_ITEM(result, i) == NULL)) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
            Py_DECREF(result);
            PyMem_Free(buf);
            return NULL;
                                         ...
                                         __attribute__((__unused__)))
{
    ...
    void (^block)(id, const char*, NSUInteger, NSString*) =
        ^(id _Nullable self, const char* bytes, NSUInteger length, NSString* key) {
          ...
          v3 = id_to_python(key);
          if (unlikely(v3 == NULL)) // LCOV_BR_EXCL_LINE
              goto error;           // LCOV_EXCL_LINE
...
PyObjC_setup_nscoder(PyObject* module __attribute__((__unused__)))
{
    ...
    if (unlikely(
            PyObjC_RegisterMethodMapping(classNSCoder, // LCOV_BR_EXCL_LINE
    ...
    if (unlikely(PyObjC_RegisterMethodMapping(classNSCoder, // LCOV_BR_EXCL_LINE
