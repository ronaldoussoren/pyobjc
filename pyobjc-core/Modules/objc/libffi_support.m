...
PyObjCFFI_Setup(PyObject* m __attribute__((__unused__)))
{
    ...
    if (unlikely(array_types == NULL)) { // LCOV_BR_EXCL_LINE
        return -1;                       // LCOV_EXCL_LINE
    }
    struct_types = PyDict_New();
    if (unlikely(struct_types == NULL)) { // LCOV_BR_EXCL_LINE
        return -1;                        // LCOV_EXCL_LINE
    }
    return 0;
}
...
num_struct_fields(const char* orig_argtype)
{
    ...
    while (*argtype != _C_STRUCT_E) {
        if (unlikely(*argtype == '"')) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
            /* Skip field name */
            argtype++;
        ...
        if (unlikely(argtype == NULL)) { // LCOV_BR_EXCL_LINE
            return -1;                   // LCOV_EXCL_LINE
        }
        res++;
    }
...
static ffi_type* _Nullable array_to_ffi_type(const char* argtype)
{
    ...
    if (unlikely(type->elements == NULL)) { // LCOV_BR_EXCL_LINE
        // LCOV_EXCL_START
        Py_DECREF(typestr);
        PyMem_Free(type);
        PyErr_NoMemory();
    ...
    if (unlikely(type->elements[0] == NULL)) { // LCOV_BR_EXCL_LINE
...
static ffi_type* _Nullable struct_to_ffi_type(const char* argtype)
{
    ...
    if (unlikely(field_count == -1)) { // LCOV_BR_EXCL_LINE
        /* NOTE: this cannot happen, the code calling this function has
    ...
    if (*curtype == '=') {
        ...
        while (*curtype != _C_STRUCT_E) {
            if (unlikely(*curtype == '"')) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                /* Skip field name */
                curtype++;
            ...
            if (unlikely(type->elements[field_count] == NULL)) { // LCOV_BR_EXCL_LINE
                /* NOTE: Can only happen when running out of memory, or
                 * when the type string is invalid. The type string has
                 * already been checked by our caller.
                 */
                // LCOV_EXCL_START
            ...
            if (unlikely(curtype == NULL)) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                Py_DECREF(typestr);
                PyMem_Free(type->elements);
                return NULL;
...
ffi_type* _Nullable PyObjCFFI_Typestr2FFI(const char* argtype)
{
    ...
    default:
        /* Cannot happen, see earlier explanation */
        // LCOV_EXCL_START
...
extract_count(int arrayarg, const char* type, void* pvalue)
{
    ...
    case _C_PTR:
        ...
        if (unlikely(strncmp(type + 1, "{_CFRange=qq}", sizeof("{_CFRange=qq}") - 1)
                     == 0)) {
            result = (Py_ssize_t)((*(CFRange**)pvalue)->length);
            goto done;
        }

        if (unlikely(strncmp(type + 1, "{_CFRange=ll}", sizeof("{_CFRange=ll}") - 1)
                     == 0)) {
            result = (Py_ssize_t)((*(CFRange**)pvalue)->length);
            goto done;
        }
        if (unlikely(strncmp(type + 1, "{CFRange=qq}", sizeof("{CFRange=qq}") - 1)
                     == 0)) {
            result = (Py_ssize_t)((*(CFRange**)pvalue)->length);
            goto done;
        }

        if (unlikely(strncmp(type + 1, "{CFRange=ll}", sizeof("{CFRange=ll}") - 1)
                     == 0)) {
            result = (Py_ssize_t)((*(CFRange**)pvalue)->length);
            goto done;
        }
    ...
    if (unlikely(strncmp(type, "{_CFRange=qq}", sizeof("{_CFRange=qq}") - 1) == 0)) {
        result = (Py_ssize_t)(((CFRange*)pvalue)->length);
        goto done;
    }

    if (unlikely(strncmp(type, "{_CFRange=ll}", sizeof("{_CFRange=ll}") - 1) == 0)) {
        result = (Py_ssize_t)(((CFRange*)pvalue)->length);
        goto done;
    }

    if (unlikely(strncmp(type, "{CFRange=qq}", sizeof("{CFRange=qq}") - 1) == 0)) {
        result = (Py_ssize_t)(((CFRange*)pvalue)->length);
        goto done;
    }

    if (unlikely(strncmp(type, "{CFRange=ll}", sizeof("{CFRange=ll}") - 1) == 0)) {
        result = (Py_ssize_t)(((CFRange*)pvalue)->length);
        goto done;
    }
                  ...
                  ffi_type** arglist, void** values, Py_ssize_t curarg)
{
    ...
    if (PyBytes_Check(py_format)) {
        ...
        if (unlikely(format != NULL)) { // LCOV_BR_EXCL_LINE
            encoded = py_format;
            Py_INCREF(encoded);
        ...
        PyErr_Format(PyExc_TypeError, "Unsupported format string type, %s",
                     Py_TYPE(py_format)->tp_name);
        return -1;
    }
    ...
    while (format && *format != '\0') {
        ...
        if (*format == '*') {
            ...
            if (unlikely(byref[curarg] == NULL)) { // LCOV_BR_EXCL_LINE
                // LCOV_EXCL_START
                Py_DECREF(encoded);
                return -1;
                // LCOV_EXCL_STOP
            ...
            void* _userdata)
{
    ...
    if (unlikely(rettype == NULL)) { // LCOV_BR_EXCL_LINE
        // LCOV_EXCL_START
    ...
    if (userdata->closureType == PyObjC_Method) {
        ...
        /* 'insertArg' is never true, the rest of the bridge
         * won't accept a bound method as a new selector.
         */
        assert(!insertArg);
    ...
    for (i = startArg; i < methinfo_size; i++) {
        ...
        case _C_CONST:
            if (argtype[1] == _C_PTR && argtype[2] == _C_VOID
                ...
                if (argtype[1] == _C_PTR) { // LCOV_BR_EXCL_LINE
                    resttype = argtype + 2;
                } else {
                    resttype = gCharEncoding; // LCOV_EXCL_LINE
                }
                ...
                if (unlikely(block_methinfo == NULL)) {
                    ...
                    if (methinfo->argtype[i]->callable != NULL) {
                        ...
                        if (signature != NULL) {
                            ...
                            if (unlikely(sig == NULL)) { // LCOV_BR_EXCL_LINE
                                // LCOV_EXCL_START
                                Py_DECREF(v);
                                v = NULL;
                                // LCOV_EXCL_STOP
    ...
    if (likely(!have_output)) {
        ...
        if (*rettype == _C_VOID && have_output == 1) {
            ...
            for (i = startArg; i < Py_SIZE(methinfo); i++) {
                ...
                case _C_OUT:
                    if (argtype[1] == _C_PTR) {
                        argtype += 2;
                    } else if (likely(argtype[1] == _C_CHARPTR)) { // LCOV_BR_EXCL_LINE
                ...
                switch (methinfo->argtype[i]->ptrType) {
                case PyObjC_kPointerPlain: {
                    /* XXX: Using is_uniquely_referenced is dodgy, and fails with
                     *      the freethreaded build (hence defining the value to 1 there)
                     *
                     *      What this tries to accomplish is twofold:
                     *      1) Ensure that the value stays alive long enough for
                     *         the caller to see and use it;
                     *      2) Don't retain+autorelease when the value will stay
                     *         alive.
                     *
                     *      The 'value' here is the one converted to ObjC, not the
                     *      python value we're testing here!
                     *
                     *      Just using 'retain+autorelease' is probably better,
                     *      possibly use ``objc_retainAutoreleaseReturnValue()`` from
                     *      the ARC runtime for the return value (although I'm far
                     *      from convinced that moment that this will actually do
                     *      anything useful).
                     */
                    bool is_uniquely_referenced = (
#ifdef Py_GIL_DISABLED
                        1 /* PyUnstable_Object_IsUniquelyReferenced(res) */
#else
                        Py_REFCNT(res) == 1
#endif
                    );
                    ...
                    if (argtype[0] == _C_ID) {
                        if (methinfo->argtype[i]->alreadyRetained) {
                            [**(id**)args[i] retain];
                        } else if (methinfo->argtype[i]->alreadyCFRetained) {
                            CFRetain(**(id**)args[i]);
                        } else if (is_uniquely_referenced) {
        ...
        if (*rettype != _C_VOID) {
            ...
            if (unqualified_type[0] == _C_PTR || unqualified_type[0] == _C_CHARPTR) {
                const char* resttype = unqualified_type + 1;
                if (unqualified_type[0] == _C_CHARPTR) { // LCOV_BR_EXCL_LINE
                    resttype = gCharEncoding;            // LCOV_EXCL_LINE
                } // LCOV_EXCL_LINE
                ...
                bool is_uniquely_referenced = (
#ifdef Py_GIL_DISABLED
                    1 /* PyUnstable_Object_IsUniquelyReferenced(real_res) */
#else
                    Py_REFCNT(res) == 1
#endif
                );
        ...
        for (i = startArg; i < Py_SIZE(methinfo); i++) {
            ...
            case PyObjC_kPointerPlain: {
                bool is_uniquely_referenced = (
#ifdef Py_GIL_DISABLED
                    1 /* PyUnstable_Object_IsUniquelyReferenced(PyTuple_GET_ITEM(res,
                         idx)) */
#else
                    Py_REFCNT(res) == 1
#endif
            ...
            for (i = startArg; i < Py_SIZE(methinfo); i++) {
                ...
                case _C_OUT:
                    if (argtype[1] == _C_PTR) {
                        ...
                        /* XXX: Validate that metadata builder never
          ...
          Py_ssize_t* defaultCount)
{
    ...
    if (PyObjC_is_pyfunction(callable) || PyObjC_is_pymethod(callable)) {
        ...
        *defaultCount = PyObjC_num_defaults(callable);
        if (unlikely(*defaultCount == -1)) { // LCOV_BR_EXCL_LINE
        ...
        *defaultCount     = 0;
         ...
         */
        PyErr_Format(PyExc_TypeError,
                     "Cannot create native callable for instances of type %s",
    ...
    PyObjCMethodSignature* methinfo, PyObject* callable)
{
    ...
    if (closure == NULL) {
        Py_DECREF(methinfo);
        assert(stubUserdata->callable != NULL);
                         ...
                         BOOL* variadicAllArgs)
{
    ...
    *byref_in_count = *byref_out_count = *plain_count = 0;
    ...
    for (i = argOffset; i < Py_SIZE(methinfo); i++) {
        ...
        /*
         * The size and alignment of 'argtype' can always be determined because it
         * wouldn't have been possible to create the 'methinfo' object if the
         * type encoding is not valid.
         */
        ...
        case _C_INOUT:
            if (argtype[1] == _C_PTR && PyObjCPointerWrapper_HaveWrapper(argtype + 1)) {
                ...
                // LCOV_EXCL_START
                assert(0);
                PyErr_SetString(PyObjCExc_InternalError, "byref non-pointer");
                return -1;
                // LCOV_EXCL_STOP
            }
                         ...
                         void** values)
{
    ...
    for (i = argOffset; i < meth_arg_count; i++) {
        ...
        if (unlikely(argtype[0] == _C_OUT
            ...
            case PyObjC_kDerefResultPointer:
                PyErr_SetString(PyObjCExc_Error,
                                "using 'deref_result_pointer' metadata for an argument");
                return -1;
            }
            ...
            case _C_UNION_B:
                ...
                if (*argtype == _C_ARY_B) { // LCOV_BR_EXCL_LINE
                    values[i] = &byref[i];  // LCOV_EXCL_LINE
            ...
            case _C_IN:
            case _C_CONST:
                assert(!(argtype[1] == _C_PTR && argtype[2] == _C_VOID
                         && methinfo->argtype[i]->ptrType == PyObjC_kPointerPlain));
                if (argtype[1] == _C_CHARPTR
                    || (argtype[1] == _C_PTR
                        && !PyObjCPointerWrapper_HaveWrapper(argtype + 1))) {
                    ...
                    if (argument == PyObjC_NULL || argument == Py_None) {
                        ...
                        switch (methinfo->argtype[i]->ptrType) { // LCOV_BR_EXCL_LINE
                        case PyObjC_kPointerPlain:
                        ...
                        *(PyObjC_callback_function*)arg = NULL;
                        ...
                        if (v == NULL) {
                            ...
                            closure = (PyObjC_callback_function)PyCapsule_GetPointer(
                                v, "objc.__imp__");
                            if (closure == NULL) { // LCOV_BR_EXCL_LINE
                                /* Can only get here due to bug in this extension */
                                // LCOV_EXCL_START
                                PyErr_SetString(PyExc_TypeError,
                                                "Invalid pyobjc_closure attribute");
                                return -1;
                ...
                if (argtype[0] == _C_ID && argtype[1] == '?') {
                    ...
                    if (argument == Py_None) {
                        ...
                        *(void**)arg = NULL;
                        ...
                        argbuf_cur += PyObjCRT_SizeOfType(argtype);
                        assert(argbuf_cur <= argbuf_len);
                        assert(methinfo->argtype[i]->callable != NULL);
                        *(void**)arg =
    ...
    if (have_counted_array) {
        ...
        for (i = argOffset; i < meth_arg_count; i++) {
            ...
            if (argtype[0] == _C_OUT
                ...
                /* Encode argument, maybe after allocating space */
                if (argtype[0] == _C_OUT) // LCOV_BR_EXCL_LINE
                    /* XXX: this cannot happen, remove block after fully verifying */
                    argtype++; // LCOV_EXCL_LINE
                ...
                case _C_IN:
                case _C_CONST:
                    if (argtype[0] == _C_CHARPTR || argtype[1] == _C_PTR
                        || argtype[1] == _C_CHARPTR) {
                        /* Allocate space and encode */
                        const char* resttype = argtype + 2;
                        if (argtype[0] == _C_CHARPTR || argtype[1] == _C_CHARPTR) {
                            resttype = gCharEncoding;
                        }
    ...
    if (methinfo->variadic && printf_format) {
        Py_ssize_t r;
                                          ...
                                          Py_ssize_t byref_out_count, void** argvalues)
{
    ...
    if ((*methinfo->rettype->type != _C_VOID) /* && (![methinfo isOneway]) */) {
        ...
        if (unlikely(tp == NULL)) { // LCOV_BR_EXCL_LINE
            return NULL;            // LCOV_EXCL_LINE
        }
        ...
        if (objc_result == NULL) {
            if (tp[0] == _C_ID && tp[1] == '?') {
                ...
                [v release];
                if (unlikely(objc_result == NULL)) { // LCOV_BR_EXCL_LINE
                    return NULL;                     // LCOV_EXCL_LINE
                }
            ...
            case _C_OUT:
                if (argtype[1] == _C_CHARPTR
                    ...
                    if (arg == NULL) {
                        ...
                        case PyObjC_kVariableLengthArray:
                            /* 'out/inout' variable length buffers require passing in a
                        ...
                        case PyObjC_kArrayCountInArg:
                            ...
                            break;
                        case PyObjC_kDerefResultPointer:
                            /* Cannot get here, ParseArgs already raised an exception */
                            // LCOV_EXCL_START
                            PyErr_SetString(
                                PyObjCExc_Error,
                    ...
                    if (result != NULL) {
                        if (unlikely(PyTuple_SetItem(result, py_arg++, v)
                                     < 0)) { // LCOV_BR_EXCL_LINE
                            /* XXX: Recheck the code and replace with
                             *      PyTuple_SET_ITEM + assertion.
                             */
                            // LCOV_EXCL_START
                            Py_DECREF(v);
                            goto error_cleanup;
                     ...
                     * is a C NULL pointer)
                     *
                     * XXX: I'm pretty sure this cannot happen at all,
                                     ...
                                     PyObject* const* args, size_t nargs)
{
    ...
    if (self_obj == nil) { // LCOV_BR_EXCL_LINE
                                               ...
                                               PyObject* const* args, size_t nargsf)
{
    ...
    if (unlikely(cif == NULL)) {
        cif = PyObjCFFI_CIFForSignature(methinfo);
        if (unlikely(cif == NULL)) { // LCOV_BR_EXCL_LINE
            return NULL;             // LCOV_EXCL_LINE
        }
        if (unlikely(PyObjCIMP_Check(aMeth))) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
            /* XXX: This function is only called in a vectorcall implementation for
             * a selector, not by the implementation of IMP. IMP uses a different
             * variant, check if the two can be merged.
             */
            if (unlikely(PyObjCIMP_SetCIF(aMeth, cif) == -1)) {
                PyObjCFFI_FreeCIF(cif);
                return NULL;
            }
            // LCOV_EXCL_STOP
    ...
    if (unlikely(self_obj == nil)) { // LCOV_BR_EXCL_LINE
        /* XXX: At the moment the only way to get here is a meta class
         * has a Nil Class value, which should never happen.
         */
        // LCOV_EXCL_START
...
ffi_cif* _Nullable PyObjCFFI_CIFForSignature(PyObjCMethodSignature* methinfo)
{
    ...
    if (unlikely(cl_arg_types == NULL)) { // LCOV_BR_EXCL_LINE
        // LCOV_EXCL_START
    ...
    for (i = 0; i < Py_SIZE(methinfo); i++) {
        ...
        if (unlikely(cl_arg_types[i] == NULL)) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
...
#pragma clang diagnostic pop
    ...
    if (unlikely(rv != FFI_OK)) { // LCOV_BR_EXCL_LINE
        // LCOV_EXCL_START
        PyMem_Free(cif);
        PyMem_Free(cl_arg_types);
        PyErr_Format(PyObjCExc_Error, "Cannot create FFI CIF for %s: %s",
                                    ...
                                    PyObjCFFI_ClosureFunc func, void* userdata)
{
    ...
    if (unlikely(alloc_prepped_closure // LCOV_BR_EXCL_LINE
                 (&cl, cif, &codeloc, (void*)func, userdata)
                 == -1)) {
        // LCOV_EXCL_START
...
PyObjCFFI_CallUsingInvocation(IMP method, NSInvocation* invocation)
{
    ...
    if (unlikely(arglist[0] == NULL)) { // LCOV_BR_EXCL_LINE
        return -1;                      // LCOV_EXCL_LINE
    }
    if (*typestr == _C_VOID) {
        values[0] = NULL;
        ...
        if (unlikely(values[0] == NULL)) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
            rv = -1;
            goto cleanup;
            // LCOV_EXCL_STOP
    ...
    for (i = 0; i < [signature numberOfArguments]; i++) {
        ...
        if (unlikely(arglist[i + 1] == NULL)) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
            rv = -1;
            goto cleanup;
            // LCOV_EXCL_STOP
        ...
        values[i + 1] =
            PyMem_Malloc(MAX(PyObjCRT_SizeOfType(typestr), (Py_ssize_t)sizeof(long)));
        if (unlikely(values[i + 1] == NULL)) { // LCOV_BR_EXCL_LINE
            // LCOV_EXCL_START
            rv = -1;
            goto cleanup;
            // LCOV_EXCL_STOP
