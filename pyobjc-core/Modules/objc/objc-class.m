                                     ...
                                     PyObject* _Nullable args, PyObject* _Nullable kwds)
{
    ...
    if (isCFProxyClass) {
        ...
        if (unlikely( // LCOV_BR_EXCL_LINE
                PyList_SetItem(real_bases, 0, py_super_class) < 0)) {
            // LCOV_EXCL_START
            assert(objc_class != Nil);
            (void)PyObjCClass_UnbuildClass(objc_class);
            Py_XDECREF(orig_slots);
    ...
    case 1:
        if (isCFProxyClass) {
            ...
            if (unlikely( // LCOV_BR_EXCL_LINE
                    PyDict_DelItem(dict, PyObjCNM___del__) < 0)) {
                // LCOV_EXCL_START
                if (objc_class != nil) {
                    (void)PyObjCClass_UnbuildClass(objc_class);
                }
                                                       ...
                                                       SEL sel)
{
    ...
    if (unlikely(hidden || PyErr_Occurred())) {
        Py_CLEAR(hidden);
        return NULL;
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        if (PyObjCClass_Check(base)) {
            ...
            if (m) {
                ...
                /* add to __dict__ 'cache' */
                if (unlikely( // LCOV_BR_EXCL_LINE
                        PyDict_SetItem(class_dict, name, result) == -1)) {
                    // LCOV_EXCL_START
                    Py_DECREF(result);
                    Py_CLEAR(mro);
                    return NULL;
...
PyObject* _Nullable PyObjCClass_New(Class objc_class)
{
...

    {
        ...
        if (unlikely( // LCOV_BR_EXCL_LINE
                PyDict_SetItem(dict, PyObjCNM___slots__, slots) == -1)) {
            // LCOV_EXCL_START
            Py_DECREF(hiddenSelectors);
            Py_DECREF(hiddenClassSelectors);
            Py_DECREF(metaclass);
...
PyObject* _Nullable PyObjCClass_ListProperties(PyObject* aClass)
{
    ...
    for (i = 0; i < propcount; i++) {
        ...
        if (unlikely( // LCOV_BR_EXCL_LINE
                PyDict_SetItem(item, PyObjCNM_typestr, v) == -1)) {
            // LCOV_EXCL_START
            Py_DECREF(v);
            goto error;
            // LCOV_EXCL_STOP
        ...
        if (*attr == '"') {
            ...
            if (unlikely( // LCOV_BR_EXCL_LINE
                    PyDict_SetItem(item, PyObjCNM_classname, v) == -1)) {
                // LCOV_EXCL_START
                Py_DECREF(v);
                goto error;
                // LCOV_EXCL_STOP
        ...
        while (attr && *attr != '\0') {
            ...
            case 'G':
                ...
                if (unlikely( // LCOV_BR_EXCL_LINE
                        PyDict_SetItem(item, PyObjCNM_getter, v) == -1)) {
                    // LCOV_EXCL_START
                    Py_DECREF(v);
                    goto error;
                    // LCOV_EXCL_STOP
            ...
            case 'S':
                ...
                if (unlikely( // LCOV_BR_EXCL_LINE
                        PyDict_SetItem(item, PyObjCNM_setter, v) == -1)) {
                    // LCOV_EXCL_START
                    Py_DECREF(v);
                    goto error;
                    // LCOV_EXCL_STOP
