...
static void
cls_ret_sshort_fn(ffi_cif* cif, void* resp, void** args, void* userdata)
{
    *(ffi_arg*)resp = *(signed short*)args[0];
    printf("%d: %d\n", *(signed short*)args[0], *(ffi_arg*)resp);
}
...
main(void)
{
    ...
    /* Initialize the cif */
    CHECK(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_sint16, cl_arg_types)
          == FFI_OK);
