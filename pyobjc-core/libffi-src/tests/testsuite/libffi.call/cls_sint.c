...
static void
cls_ret_sint_fn(ffi_cif* cif, void* resp, void** args, void* userdata)
{
    *(ffi_arg*)resp = *(signed int*)args[0];
    printf("%d: %d\n", *(signed int*)args[0], *(ffi_arg*)resp);
}
...
main(void)
{
    ...
    /* Initialize the cif */
    CHECK(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_sint32, cl_arg_types)
          == FFI_OK);
