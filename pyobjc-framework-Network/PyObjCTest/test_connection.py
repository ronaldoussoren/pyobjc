    ...
    class TestConnection(TestCase):
        def test_constants(self):
            ...
            self.assertIsInstance(
                Network.NW_CONNECTION_SEND_IDEMPOTENT_CONTENT, objc.objc_object
            )
            self.assertIsInstance(
                Network.NW_CONNECTION_DEFAULT_MESSAGE_CONTEXT, objc.objc_object
            )
            self.assertIsInstance(
                Network.NW_CONNECTION_FINAL_MESSAGE_CONTEXT, objc.objc_object
            )
            self.assertIsInstance(
                Network.NW_CONNECTION_DEFAULT_STREAM_CONTEXT, objc.objc_object
            )
        ...
        def test_functions(self):
            ...
            self.assertArgIsBlock(
                Network.nw_connection_set_viability_changed_handler, 1, b"vB"
            )

            self.assertArgHasType(
                Network.nw_connection_set_better_path_available_handler, 0, objc._C_ID
            )
            self.assertArgIsBlock(
                Network.nw_connection_set_better_path_available_handler, 1, b"vB"
            )

            self.assertArgHasType(
                Network.nw_connection_set_path_changed_handler, 0, objc._C_ID
            )
            self.assertArgIsBlock(
                Network.nw_connection_set_path_changed_handler, 1, b"v@"
            )
            ...
            self.assertArgHasType(
                Network.nw_connection_cancel_current_endpoint, 0, objc._C_ID
            )
