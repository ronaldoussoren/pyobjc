...
if sys.maxsize > 2 ** 32:
    ...
    class TestBrowseDescriptor (TestCase):
        ...
        def test_functions10_15(self):
            ...
            self.assertArgIsNullTerminated(Network.nw_browse_descriptor_create_bonjour_service, 0)
            self.assertArgIsIn(Network.nw_browse_descriptor_create_bonjour_service, 1)
            self.assertArgIsNullTerminated(Network.nw_browse_descriptor_create_bonjour_service, 1)

            self.assertResultIsNullTerminated(Network.nw_browse_descriptor_get_bonjour_service_type)
            self.assertResultIsNullTerminated(Network.nw_browse_descriptor_get_bonjour_service_domain)
