...
if sys.maxsize >= 2 ** 32:
    ...
    class TestNEDNSProxyManager (TestCase):
        ...
        def test_constants10_15(self):
            self.assertIsInstance(NetworkExtension.NEDNSProxyConfigurationDidChangeNotification, unicode)

        @min_os_level('10.15')
        def test_methods10_15(self):
            self.assertArgIsBlock(NetworkExtension.NEDNSProxyManager.loadFromPreferencesWithCompletionHandler_, 0, b'v@')
            self.assertArgIsBlock(NetworkExtension.NEDNSProxyManager.removeFromPreferencesWithCompletionHandler_, 0, b'v@')
            self.assertArgIsBlock(NetworkExtension.NEDNSProxyManager.saveToPreferencesWithCompletionHandler_, 0, b'v@')
