...
if sys.maxsize >= 2 ** 32:
    ...
    class TestNEAppProxyFlow(TestCase):
        ...
        def testConstants(self):
            ...
            self.assertEqual(
                NetworkExtension.NETunnelProviderErrorNetworkSettingsFailed, 3
            )

            self.assertEqual(
                NetworkExtension.NETunnelProviderRoutingMethodDestinationIP, 1
            )
            self.assertEqual(
                NetworkExtension.NETunnelProviderRoutingMethodSourceApplication, 2
            )
            self.assertEqual(
                NetworkExtension.NETunnelProviderRoutingMethodNetworkRule, 3
            )
