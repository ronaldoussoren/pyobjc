...
def createURL(path):
    ...
    return Cocoa.CFURLCreateFromFileSystemRepresentation(None, path, len(path), False)
...
def StampWithPDFDocument(context, sourcePDFDoc, stampFileDoc, stampMediaRect):
    ...
    for i in range(1, numPages + 1):
        ...
        Quartz.CGContextTranslateCTM(
            context, pageRect.size.width / 2, pageRect.size.height / 2
        )
        ...
        Quartz.CGContextTranslateCTM(
            context, -stampMediaRect.size.width / 2, -stampMediaRect.size.height / 2
        )
        # Now draw the document to stamp with on top of original content.
        Quartz.CGContextDrawPDFDocument(context, stampMediaRect, stampFileDoc, 1)
...
def createStampedFileWithFile(inURL, stampURL, outURL):
    ...
    StampWithPDFDocument(
        pdfContext, sourceFileData.pdfDoc, stampFileData.pdfDoc, stampFileData.mediaRect
    )
...
def main(args=None):
    ...
    stampFileName = os.path.join(os.path.dirname(__file__), "confidential.pdf")
