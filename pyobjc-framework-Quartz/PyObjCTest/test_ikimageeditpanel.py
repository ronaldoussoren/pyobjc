...
class TestIKImageEditPanel(TestCase):
    ...
    def no_testProtocols(self):
        self.assertIsInstance(
            objc.protocolNamed("IKImageEditPanel"), objc.formal_protocol
        )
        self.assertIsInstance(
            objc.protocolNamed("IKImageEditPanelDataSource"), objc.formal_protocol
        )

    @min_os_level("10.5")
    def testProtocolMethods(self):
        self.assertArgHasType(
            TestIKImageEditPanelHelper.thumbnailWithMaximumSize_, 0, NSSize.__typestr__
        )
