...
class TestCIImageProcessor(TestCase):
    ...
    def testMethods(self):
        self.assertResultIsBOOL(
            CIImageProcessorKernel.processWithInputs_arguments_output_error_
        )
        self.assertArgIsOut(
            CIImageProcessorKernel.processWithInputs_arguments_output_error_, 3
        )

        self.assertResultHasType(
            CIImageProcessorKernel.roiForInput_arguments_outputRect_, CGRect.__typestr__
        )
        self.assertArgHasType(
            CIImageProcessorKernel.roiForInput_arguments_outputRect_,
            2,
            CGRect.__typestr__,
        )

        self.assertArgIsOut(
            CIImageProcessorKernel.applyWithExtent_inputs_arguments_error_, 3
        )
