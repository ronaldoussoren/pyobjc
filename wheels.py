"""
Helper script that will create python wheels for all components of PyObjC.

Usage:
    pythonX.Y wheel.py DIR

*DIR* is the directory where wheels will be stored. That directory must exist.

NOTE: ``pyobjc-core`` must be installed when running this script, otherwise
it will be build a lot!
"""
...
def build_project(project, wheel_dir):
    ...
    status = subprocess.call(
        [sys.executable, "setup.py", "bdist_wheel", "-d", wheel_dir],
        cwd=proj_dir)
    if status != 0:
...
def main():
    ...
    if not os.path.exists(wheel_dir):
        raise SystemExit('Wheel directory %r does not exist'%(wheel_dir))

    wheel_dir = os.path.abspath(wheel_dir)
    print(wheel_dir)

    for project in ['pyobjc-core'] + sorted_framework_wrappers():
        ok = build_project(project, wheel_dir)
        if not ok:
            break
