...
#
# Last update: Sat Mar 21 15:30:32 2020
#
# flake8: noqa
...
misc.update({})
functions = {
    "DASessionGetTypeID": (sel32or64(b"L", b"Q"),),
    "DADiskClaim": (
        sel32or64(b"v^{__DADisk=}L^?^v^?^v", b"v^{__DADisk=}I^?^v^?^v"),
        "",
        {
            "arguments": {
                2: {
    ...
    "DADiskSetOptions": (sel32or64(b"i^{__DADisk=}LZ", b"i^{__DADisk=}IZ"),),
    "DAApprovalSessionUnscheduleFromRunLoop": (
        b"v^{__DASession=}^{__CFRunLoop=}^{__CFString=}",
    ),
    "DASessionSetDispatchQueue": (b"v^{__DASession=}^{dispatch_queue_s=}",),
    ...
    "DADiskGetOptions": (sel32or64(b"L^{__DADisk=}", b"I^{__DADisk=}"),),
    "DADiskMountWithArguments": (
        sel32or64(
            b"v^{__DADisk=}^{__CFURL=}L^?^v^^{__CFString=}",
    ...
    "DAApprovalSessionGetTypeID": (sel32or64(b"L", b"Q"),),
    "DADiskRename": (
        sel32or64(
            b"v^{__DADisk=}^{__CFString=}L^?^v", b"v^{__DADisk=}^{__CFString=}I^?^v"
        ),
        "",
        {
            "arguments": {
    ...
    "DARegisterDiskPeekCallback": (
        sel32or64(
            b"v^{__DASession=}^{__CFDictionary=}l^?^v",
    ...
    "DADiskEject": (
        sel32or64(b"v^{__DADisk=}L^?^v", b"v^{__DADisk=}I^?^v"),
        "",
        {
            "arguments": {
                2: {
    ...
    "DADiskMount": (
        sel32or64(b"v^{__DADisk=}^{__CFURL=}L^?^v", b"v^{__DADisk=}^{__CFURL=}I^?^v"),
        "",
        {
            "arguments": {
                3: {
