...
def doSubstitutions(dirName, aName, options):
    if options.doReverse:
        ...
        quoteCleaner = REVERSEQUOTETRANSLATOR
    else:
        translator = FORWARDTRANSLATOR
        quoteCleaner = FORWARDQUOTETRANSLATOR
    ...
    if encoding is None:
        ...
        if options.makeWorking and (extension in WORKINGCOPYFILES) and not options.doReverse and aName.split("_", 1)[0] == 'xcPROJECTNAMExc':
            ...
            cleanQuotes = None
            if os.path.splitext(path)[1] in extensionsThatRequireQuoting:
                cleanQuotes = quoteCleaner
            doFileSubstitution(path, encoding, translator, cleanQuotes)
...
FORWARDQUOTETRANSLATOR = getQuoteSub(REVERSE)
REVERSEQUOTETRANSLATOR = None
