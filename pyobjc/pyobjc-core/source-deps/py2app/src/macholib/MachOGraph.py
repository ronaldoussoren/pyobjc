...
class MachOGraph(ObjectGraph):
    """
    Graph data structure of Mach-O dependencies for a given architecture
    """
    def __init__(self, arch, debug=0, graph=None, env=None, executable_path=None):
    ...
    def run_file(self, pathname, caller=None):
        print "Graphing %s (%s)" % (pathname, self.arch)
        ...
        if m is None:
            ...
            m = MachO(pathname)
        archs = [arch for arch in m.walkArchs() if arch.name() == self.arch]
        if len(archs) == 0:
            raise ValueError, '%r does not contain architecture %s' % (pathname, self.arch)
        if len(archs) != 1:
            raise ValueError, '%r contains multiple entries for architecture %s' % (pathname, self.arch)
    ...
    def scan_node(self, node):
        ...
        a = getattr(node, 'getArch', None)
        if a:
            for idx, name, filename in a(self.arch).walkRelocatables():
