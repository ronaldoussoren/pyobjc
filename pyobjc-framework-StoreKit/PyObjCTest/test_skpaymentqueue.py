...
class TestSKPaymentQueueHelper (StoreKit.NSObject):
    def paymentQueue_shouldContinueTransaction_inStorefront_(self, a, b, c): return 1

class TestSKPaymentQueue (TestCase):
    def test_methods(self):
        self.assertResultIsBOOL(StoreKit.SKPaymentQueue.canMakePayments)

        self.assertResultIsBOOL(TestSKPaymentQueueHelper.paymentQueue_shouldContinueTransaction_inStorefront_)
    ...
    def test_protocols10_15(self):
        self.assertIsInstance(objc.protocolNamed("SKPaymentQueueDelegate"), objc.formal_protocol)
