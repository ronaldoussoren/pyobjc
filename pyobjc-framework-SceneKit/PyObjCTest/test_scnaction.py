...
if os_level_key(os_release()) < os_level_key('10.12') or sys.maxsize >= 2**32:
    ...
    class TestSCNActionHelper (SceneKit.NSObject):
        def runAction_completionHandler_(self, a, h): pass
        def runAction_forKey_completionHandler_(self, a, k, h): pass
        def hasActions(self): return 1
    ...
    class TestSCNAction (TestCase):
        ...
        def testMethods(self):
            self.assertResultIsBlock(SceneKit.SCNAction.timingFunction, SCNActionTimingFunction)
            self.assertArgIsBlock(SceneKit.SCNAction.setTimingFunction_, 0, SCNActionTimingFunction)
            self.assertArgIsBOOL(SceneKit.SCNAction.rotateToX_y_z_duration_shortestUnitArc_, 4)
