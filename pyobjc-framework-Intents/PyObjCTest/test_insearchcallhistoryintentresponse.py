...
if sys.maxsize > 2 ** 32:
    ...
    class TestINSearchCallHistoryIntentResponse(TestCase):
        ...
        def testConstants(self):
            self.assertEqual(
                Intents.INSearchCallHistoryIntentResponseCodeUnspecified, 0
            )
            self.assertEqual(Intents.INSearchCallHistoryIntentResponseCodeReady, 1)
            self.assertEqual(
                Intents.INSearchCallHistoryIntentResponseCodeContinueInApp, 2
            )
            self.assertEqual(Intents.INSearchCallHistoryIntentResponseCodeFailure, 3)
            self.assertEqual(
                Intents.INSearchCallHistoryIntentResponseCodeFailureRequiringAppLaunch,
                4,
            )
            self.assertEqual(
