...
class TestNSLinguisticTagger(TestCase):
    ...
    def testMethods(self):
        self.assertArgHasType(
            NSLinguisticTagger.orthographyAtIndex_effectiveRange_,
            1,
            b"o^" + NSRange.__typestr__,
        )

        self.assertArgIsBlock(
            NSLinguisticTagger.enumerateTagsInRange_scheme_options_usingBlock_,
            3,
            b"v@" + NSRange.__typestr__ + NSRange.__typestr__ + b"o^" + objc._C_NSBOOL,
        )

        self.assertArgHasType(
            NSLinguisticTagger.tagAtIndex_scheme_tokenRange_sentenceRange_,
            2,
            b"o^" + NSRange.__typestr__,
        )
        self.assertArgHasType(
        ...
        self.assertArgHasType(
            NSLinguisticTagger.tagsInRange_scheme_options_tokenRanges_, 3, b"o^@"
        )

        self.assertArgHasType(
            NSLinguisticTagger.possibleTagsAtIndex_scheme_tokenRange_sentenceRange_scores_,
            2,
            b"o^" + NSRange.__typestr__,
        )
        self.assertArgHasType(
        ...
        self.assertArgIsOut(
            NSString.linguisticTagsInRange_scheme_options_orthography_tokenRanges_, 4
        )

        self.assertArgIsBlock(
            NSString.enumerateLinguisticTagsInRange_scheme_options_orthography_usingBlock_,
            4,
            b"v@" + NSRange.__typestr__ + NSRange.__typestr__ + b"o^" + objc._C_NSBOOL,
        )
    ...
    def testMethods10_13(self):
        self.assertArgHasType(
            NSLinguisticTagger.enumerateTagsInRange_unit_scheme_options_usingBlock_,
            0,
            NSRange.__typestr__,
        )
        self.assertArgIsBlock(
            NSLinguisticTagger.enumerateTagsInRange_unit_scheme_options_usingBlock_,
            4,
            b"v@" + NSRange.__typestr__ + b"o^Z",
        )
        self.assertArgHasType(
            NSLinguisticTagger.tagAtIndex_unit_scheme_tokenRange_,
            3,
            b"o^" + NSRange.__typestr__,
        )

        self.assertArgHasType(
            NSLinguisticTagger.tagsInRange_unit_scheme_options_tokenRanges_,
            0,
            NSRange.__typestr__,
        )
        self.assertArgIsOut(
        ...
        self.assertArgHasType(
            NSLinguisticTagger.enumerateTagsForString_range_unit_scheme_options_orthography_usingBlock_,
            1,
            NSRange.__typestr__,
        )
        self.assertArgIsBlock(
