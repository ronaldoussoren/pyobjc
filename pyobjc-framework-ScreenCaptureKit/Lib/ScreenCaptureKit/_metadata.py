# This file is generated by objective.metadata
#
# Last update: Sun Jun 21 12:25:21 2026
#
# flake8: noqa

import objc, sys
from typing import NewType

if sys.maxsize > 2**32:

    def sel32or64(a, b):
        return b

else:

    def sel32or64(a, b):
        return a


if objc.arch == "arm64":

    def selAorI(a, b):
        return a

else:

    def selAorI(a, b):
        return b


misc = {}
constants = """$SCStreamErrorDomain$SCStreamFrameInfoBoundingRect$SCStreamFrameInfoContentRect$SCStreamFrameInfoContentScale$SCStreamFrameInfoDirtyRects$SCStreamFrameInfoDisplayTime$SCStreamFrameInfoPresenterOverlayContentRect$SCStreamFrameInfoScaleFactor$SCStreamFrameInfoScreenRect$SCStreamFrameInfoStatus$SCStreamFrameInfoVideoOrientation$"""
enums = """$RP_REPORTING_END_REASON_SUMMARY_EVENT@2$RP_REPORTING_SCREENSHOT_EVENT@3$RP_REPORTING_SUMMARY_EVENT@1$SCCaptureDynamicRangeHDRCanonicalDisplay@2$SCCaptureDynamicRangeHDRLocalDisplay@1$SCCaptureDynamicRangeSDR@0$SCCaptureResolutionAutomatic@0$SCCaptureResolutionBest@1$SCCaptureResolutionNominal@2$SCContentSharingPickerModeMultipleApplications@8$SCContentSharingPickerModeMultipleWindows@2$SCContentSharingPickerModeSingleApplication@4$SCContentSharingPickerModeSingleDisplay@16$SCContentSharingPickerModeSingleWindow@1$SCFrameStatusBlank@2$SCFrameStatusComplete@0$SCFrameStatusIdle@1$SCFrameStatusStarted@4$SCFrameStatusStopped@5$SCFrameStatusSuspended@3$SCPresenterOverlayAlertSettingAlways@2$SCPresenterOverlayAlertSettingNever@1$SCPresenterOverlayAlertSettingSystem@0$SCRecordingEditorModePreview@0$SCRecordingEditorModeShare@1$SCScreenshotDisplayIntentCanonical@0$SCScreenshotDisplayIntentLocal@1$SCScreenshotDynamicRangeHDR@1$SCScreenshotDynamicRangeSDR@0$SCScreenshotDynamicRangeSDRAndHDR@2$SCShareableContentStyleApplication@3$SCShareableContentStyleDisplay@2$SCShareableContentStyleNone@0$SCShareableContentStyleWindow@1$SCStreamConfigurationPresetCaptureHDRRecordingPreservedSDRHDR10@4$SCStreamConfigurationPresetCaptureHDRScreenshotCanonicalDisplay@3$SCStreamConfigurationPresetCaptureHDRScreenshotLocalDisplay@2$SCStreamConfigurationPresetCaptureHDRStreamCanonicalDisplay@1$SCStreamConfigurationPresetCaptureHDRStreamLocalDisplay@0$SCStreamErrorAttemptToConfigState@-3810$SCStreamErrorAttemptToStartStreamState@-3807$SCStreamErrorAttemptToStopStreamState@-3808$SCStreamErrorAttemptToUpdateFilterState@-3809$SCStreamErrorFailedApplicationConnectionInterrupted@-3805$SCStreamErrorFailedApplicationConnectionInvalid@-3804$SCStreamErrorFailedNoMatchingApplicationContext@-3806$SCStreamErrorFailedToStart@-3802$SCStreamErrorFailedToStartAudioCapture@-3818$SCStreamErrorFailedToStartMicrophoneCapture@-3820$SCStreamErrorFailedToStopAudioCapture@-3819$SCStreamErrorInsufficientStorage@-3822$SCStreamErrorInternalError@-3811$SCStreamErrorInvalidParameter@-3812$SCStreamErrorMissingBackgroundMode@-3824$SCStreamErrorMissingEntitlements@-3803$SCStreamErrorNoCaptureSource@-3815$SCStreamErrorNoDisplayList@-3814$SCStreamErrorNoWindowList@-3813$SCStreamErrorNotSupported@-3823$SCStreamErrorRemovingStream@-3816$SCStreamErrorSystemStoppedStream@-3821$SCStreamErrorUserDeclined@-3801$SCStreamErrorUserStopped@-3817$SCStreamOutputTypeAudio@1$SCStreamOutputTypeMicrophone@2$SCStreamOutputTypeScreen@0$SCStreamTypeDisplay@1$SCStreamTypeWindow@0$"""
misc.update(
    {
        "SCStreamConfigurationPreset": NewType("SCStreamConfigurationPreset", int),
        "SCRecordingEditorMode": NewType("SCRecordingEditorMode", int),
        "SCCaptureResolutionType": NewType("SCCaptureResolutionType", int),
        "SCCaptureDynamicRange": NewType("SCCaptureDynamicRange", int),
        "SCContentSharingPickerMode": NewType("SCContentSharingPickerMode", int),
        "SCPresenterOverlayAlertSetting": NewType(
            "SCPresenterOverlayAlertSetting", int
        ),
        "SCStreamErrorCode": NewType("SCStreamErrorCode", int),
        "SCStreamType": NewType("SCStreamType", int),
        "SCStreamOutputType": NewType("SCStreamOutputType", int),
        "SCShareableContentStyle": NewType("SCShareableContentStyle", int),
        "SCScreenshotDisplayIntent": NewType("SCScreenshotDisplayIntent", int),
        "SCScreenshotDynamicRange": NewType("SCScreenshotDynamicRange", int),
        "SCFrameStatus": NewType("SCFrameStatus", int),
    }
)
misc.update({"SCStreamFrameInfo": NewType("SCStreamFrameInfo", str)})
misc.update({})
r = objc.registerMetaDataForSelector
objc._updatingMetadata(True)
try:
    r(
        b"NSObject",
        b"clipBufferingOutput:didFailWithError:",
        {
            "required": False,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"clipBufferingOutputDidStartBuffering:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"clipBufferingOutputDidStopBuffering:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"contentSharingPicker:didCancelForStream:",
        {
            "required": True,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"contentSharingPicker:didUpdateWithFilter:forStream:",
        {
            "required": True,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}, 4: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"contentSharingPickerStartDidFailWithError:",
        {"required": True, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"outputVideoEffectDidFailForStream:withError:",
        {
            "required": False,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"outputVideoEffectDidStartForStream:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"outputVideoEffectDidStopForStream:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"recordingEditor:didFailWithError:",
        {
            "required": False,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"recordingEditorDidDismiss:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"recordingOutput:didFailWithError:",
        {
            "required": False,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"recordingOutputDidFinishRecording:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"recordingOutputDidStartRecording:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"stream:didOutputSampleBuffer:ofType:",
        {
            "required": False,
            "retval": {"type": b"v"},
            "arguments": {
                2: {"type": b"@"},
                3: {"type": b"^{opaqueCMSampleBuffer=}"},
                4: {"type": b"q"},
            },
        },
    )
    r(
        b"NSObject",
        b"stream:didStopWithError:",
        {
            "required": False,
            "retval": {"type": b"v"},
            "arguments": {2: {"type": b"@"}, 3: {"type": b"@"}},
        },
    )
    r(
        b"NSObject",
        b"streamDidBecomeActive:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"NSObject",
        b"streamDidBecomeInactive:",
        {"required": False, "retval": {"type": b"v"}, "arguments": {2: {"type": b"@"}}},
    )
    r(
        b"SCClipBufferingOutput",
        b"exportClipToURL:duration:completionHandler:",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(b"SCContentFilter", b"includeMenuBar", {"retval": {"type": b"Z"}})
    r(b"SCContentFilter", b"isCameraEnabled", {"retval": {"type": b"Z"}})
    r(b"SCContentFilter", b"isMicrophoneEnabled", {"retval": {"type": b"Z"}})
    r(b"SCContentFilter", b"setIncludeMenuBar:", {"arguments": {2: {"type": b"Z"}}})
    r(b"SCContentSharingPicker", b"isActive", {"retval": {"type": b"Z"}})
    r(b"SCContentSharingPicker", b"isAvailable", {"retval": {"type": b"Z"}})
    r(b"SCContentSharingPicker", b"setActive:", {"arguments": {2: {"type": b"Z"}}})
    r(
        b"SCContentSharingPickerConfiguration",
        b"allowsChangingSelectedContent",
        {"retval": {"type": b"Z"}},
    )
    r(
        b"SCContentSharingPickerConfiguration",
        b"setAllowsChangingSelectedContent:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCContentSharingPickerConfiguration",
        b"setShowsCameraControl:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCContentSharingPickerConfiguration",
        b"setShowsMicrophoneControl:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCContentSharingPickerConfiguration",
        b"showsCameraControl",
        {"retval": {"type": b"Z"}},
    )
    r(
        b"SCContentSharingPickerConfiguration",
        b"showsMicrophoneControl",
        {"retval": {"type": b"Z"}},
    )
    r(
        b"SCRecordingEditor",
        b"presentFromWindow:completionHandler:",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(
        b"SCRecordingEditor",
        b"presentFromWindow:mode:completionHandler:",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(b"SCRecordingOutput", b"recordedDuration", {"retval": {"type": b"{CMTime=qiIq}"}})
    r(
        b"SCRecordingOutputConfiguration",
        b"mixesAudioWithMicrophone",
        {"retval": {"type": b"Z"}},
    )
    r(
        b"SCRecordingOutputConfiguration",
        b"setMixesAudioWithMicrophone:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(b"SCScreenshotConfiguration", b"ignoreClipping", {"retval": {"type": b"Z"}})
    r(b"SCScreenshotConfiguration", b"ignoreShadows", {"retval": {"type": b"Z"}})
    r(b"SCScreenshotConfiguration", b"includeChildWindows", {"retval": {"type": b"Z"}})
    r(
        b"SCScreenshotConfiguration",
        b"setIgnoreClipping:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCScreenshotConfiguration",
        b"setIgnoreShadows:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCScreenshotConfiguration",
        b"setIncludeChildWindows:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCScreenshotConfiguration",
        b"setShowsCursor:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(b"SCScreenshotConfiguration", b"showsCursor", {"retval": {"type": b"Z"}})
    r(
        b"SCScreenshotManager",
        b"captureImageInRect:completionHandler:",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"^{CGImage=}"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCScreenshotManager",
        b"captureImageWithFilter:configuration:completionHandler:",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"^{CGImage=}"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCScreenshotManager",
        b"captureSampleBufferWithFilter:configuration:completionHandler:",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"^{opaqueCMSampleBuffer=}"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCScreenshotManager",
        b"captureScreenshotWithFilter:configuration:completionHandler:",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCScreenshotManager",
        b"captureScreenshotWithRect:configuration:completionHandler:",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCShareableContent",
        b"getCurrentProcessShareableContentWithCompletionHandler:",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCShareableContent",
        b"getShareableContentExcludingDesktopWindows:onScreenWindowsOnly:completionHandler:",
        {
            "arguments": {
                2: {"type": b"Z"},
                3: {"type": b"Z"},
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                },
            }
        },
    )
    r(
        b"SCShareableContent",
        b"getShareableContentExcludingDesktopWindows:onScreenWindowsOnlyAboveWindow:completionHandler:",
        {
            "arguments": {
                2: {"type": b"Z"},
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                },
            }
        },
    )
    r(
        b"SCShareableContent",
        b"getShareableContentExcludingDesktopWindows:onScreenWindowsOnlyBelowWindow:completionHandler:",
        {
            "arguments": {
                2: {"type": b"Z"},
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                },
            }
        },
    )
    r(
        b"SCShareableContent",
        b"getShareableContentWithCompletionHandler:",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"@"},
                        },
                    }
                }
            }
        },
    )
    r(
        b"SCStream",
        b"addClipBufferingOutput:error:",
        {"retval": {"type": b"Z"}, "arguments": {3: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"addRecordingOutput:error:",
        {"retval": {"type": b"Z"}, "arguments": {3: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"addStreamOutput:type:sampleHandlerQueue:error:",
        {"retval": {"type": b"Z"}, "arguments": {5: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"addVideoEffectOutput:error:",
        {"retval": {"type": b"Z"}, "arguments": {3: {"type_modifier": b"o"}}},
    )
    r(b"SCStream", b"isCapturing", {"retval": {"type": b"Z"}})
    r(
        b"SCStream",
        b"removeClipBufferingOutput:error:",
        {"retval": {"type": b"Z"}, "arguments": {3: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"removeRecordingOutput:error:",
        {"retval": {"type": b"Z"}, "arguments": {3: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"removeStreamOutput:type:error:",
        {"retval": {"type": b"Z"}, "arguments": {4: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"removeVideoEffectOutput:error:",
        {"retval": {"type": b"Z"}, "arguments": {3: {"type_modifier": b"o"}}},
    )
    r(
        b"SCStream",
        b"startCaptureWithCompletionHandler:",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(
        b"SCStream",
        b"stopCaptureWithCompletionHandler:",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(
        b"SCStream",
        b"updateConfiguration:completionHandler:",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(
        b"SCStream",
        b"updateContentFilter:completionHandler:",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"@"}},
                    }
                }
            }
        },
    )
    r(b"SCStreamConfiguration", b"captureMicrophone", {"retval": {"type": b"Z"}})
    r(b"SCStreamConfiguration", b"capturesAudio", {"retval": {"type": b"Z"}})
    r(b"SCStreamConfiguration", b"capturesShadowsOnly", {"retval": {"type": b"Z"}})
    r(
        b"SCStreamConfiguration",
        b"excludesCurrentProcessAudio",
        {"retval": {"type": b"Z"}},
    )
    r(b"SCStreamConfiguration", b"ignoreGlobalClipDisplay", {"retval": {"type": b"Z"}})
    r(
        b"SCStreamConfiguration",
        b"ignoreGlobalClipSingleWindow",
        {"retval": {"type": b"Z"}},
    )
    r(b"SCStreamConfiguration", b"ignoreShadowsDisplay", {"retval": {"type": b"Z"}})
    r(
        b"SCStreamConfiguration",
        b"ignoreShadowsSingleWindow",
        {"retval": {"type": b"Z"}},
    )
    r(b"SCStreamConfiguration", b"includeChildWindows", {"retval": {"type": b"Z"}})
    r(
        b"SCStreamConfiguration",
        b"minimumFrameInterval",
        {"retval": {"type": "{CMTime=qiIq}"}},
    )
    r(b"SCStreamConfiguration", b"preservesAspectRatio", {"retval": {"type": b"Z"}})
    r(b"SCStreamConfiguration", b"scalesToFit", {"retval": {"type": b"Z"}})
    r(
        b"SCStreamConfiguration",
        b"setCaptureMicrophone:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setCapturesAudio:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setCapturesShadowsOnly:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setExcludesCurrentProcessAudio:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setIgnoreGlobalClipDisplay:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setIgnoreGlobalClipSingleWindow:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setIgnoreShadowsDisplay:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setIgnoreShadowsSingleWindow:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setIncludeChildWindows:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setMinimumFrameInterval:",
        {"arguments": {2: {"type": "{CMTime=qiIq}"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setPreservesAspectRatio:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(b"SCStreamConfiguration", b"setScalesToFit:", {"arguments": {2: {"type": b"Z"}}})
    r(
        b"SCStreamConfiguration",
        b"setShouldBeOpaque:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(
        b"SCStreamConfiguration",
        b"setShowMouseClicks:",
        {"arguments": {2: {"type": b"Z"}}},
    )
    r(b"SCStreamConfiguration", b"setShowsCursor:", {"arguments": {2: {"type": b"Z"}}})
    r(b"SCStreamConfiguration", b"shouldBeOpaque", {"retval": {"type": b"Z"}})
    r(b"SCStreamConfiguration", b"showMouseClicks", {"retval": {"type": b"Z"}})
    r(b"SCStreamConfiguration", b"showsCursor", {"retval": {"type": b"Z"}})
    r(b"SCWindow", b"isActive", {"retval": {"type": b"Z"}})
    r(b"SCWindow", b"isOnScreen", {"retval": {"type": b"Z"}})
finally:
    objc._updatingMetadata(False)

objc.registerNewKeywordsFromSelector("SCClipBufferingOutput", b"initWithDelegate:")
objc.registerNewKeywordsFromSelector(
    "SCContentFilter", b"initWithDesktopIndependentWindow:"
)
objc.registerNewKeywordsFromSelector(
    "SCContentFilter", b"initWithDisplay:excludingApplications:exceptingWindows:"
)
objc.registerNewKeywordsFromSelector(
    "SCContentFilter", b"initWithDisplay:excludingWindows:"
)
objc.registerNewKeywordsFromSelector(
    "SCContentFilter", b"initWithDisplay:includingApplications:exceptingWindows:"
)
objc.registerNewKeywordsFromSelector(
    "SCContentFilter", b"initWithDisplay:includingWindows:"
)
objc.registerNewKeywordsFromSelector("SCContentSharingPicker", b"init")
objc.registerNewKeywordsFromSelector("SCDisplay", b"init")
objc.registerNewKeywordsFromSelector("SCRecordingEditor", b"init")
objc.registerNewKeywordsFromSelector("SCRecordingEditor", b"initWithURL:")
objc.registerNewKeywordsFromSelector(
    "SCRecordingOutput", b"initWithConfiguration:delegate:"
)
objc.registerNewKeywordsFromSelector("SCRunningApplication", b"init")
objc.registerNewKeywordsFromSelector("SCScreenshotManager", b"init")
objc.registerNewKeywordsFromSelector("SCShareableContent", b"init")
objc.registerNewKeywordsFromSelector("SCStream", b"init")
objc.registerNewKeywordsFromSelector(
    "SCStream", b"initWithFilter:configuration:delegate:"
)
objc.registerNewKeywordsFromSelector("SCVideoEffectOutput", b"init")
objc.registerNewKeywordsFromSelector("SCVideoEffectOutput", b"initWithCameraDevice:")
objc.registerNewKeywordsFromSelector("SCWindow", b"init")

objc.registerNewKeywords(
    "SCStreamConfiguration", ("preset",), "streamConfigurationWithPreset_"
)
expressions = {}
formal_protocols = {
    "SCClipBufferingOutputDelegate": "SCClipBufferingOutputDelegate",
    "SCContentSharingPickerObserver": "SCContentSharingPickerObserver",
    "SCRecordingEditorDelegate": "SCRecordingEditorDelegate",
    "SCRecordingOutputDelegate": "SCRecordingOutputDelegate",
    "SCStreamDelegate": "SCStreamDelegate",
    "SCStreamOutput": "SCStreamOutput",
}

# END OF FILE
