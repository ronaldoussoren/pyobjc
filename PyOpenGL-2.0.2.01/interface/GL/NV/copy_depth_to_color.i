/*
# AUTOGENERATED DO NOT EDIT

# If you edit this file, delete the AUTOGENERATED line to prevent re-generation
# BUILD api_versions [0x001]
*/

%module copy_depth_to_color

#define __version__ "$Revision: 1.1.2.1 $"
#define __date__ "$Date: 2004/11/15 07:38:07 $"
#define __api_version__ API_VERSION
#define __author__ "PyOpenGL Developers <pyopengl-devel@lists.sourceforge.net>"
#define __doc__ ""

%{
/**
 *
 * GL.NV.copy_depth_to_color Module for PyOpenGL
 * 
 * Authors: PyOpenGL Developers <pyopengl-devel@lists.sourceforge.net>
 * 
***/
%}

%include util.inc
%include gl_exception_handler.inc

%{
#ifdef CGL_PLATFORM
# include <OpenGL/glext.h>
#else
# include <GL/glext.h>
#endif

#if !EXT_DEFINES_PROTO || !defined(GL_NV_copy_depth_to_color)

#endif
%}

/* FUNCTION DECLARATIONS */


/* CONSTANT DECLARATIONS */
#define    GL_DEPTH_STENCIL_TO_RGBA_NV 0x886E
#define    GL_DEPTH_STENCIL_TO_BGRA_NV 0x886F


%{
static char *proc_names[] =
{
#if !EXT_DEFINES_PROTO || !defined(GL_NV_copy_depth_to_color)

#endif
	NULL
};

#define glInitCopyDepthToColorNV() InitExtension("GL_NV_copy_depth_to_color", proc_names)
%}

int glInitCopyDepthToColorNV();
DOC(glInitCopyDepthToColorNV, "glInitCopyDepthToColorNV() -> bool")

%{
PyObject *__info()
{
	if (glInitCopyDepthToColorNV())
	{
		PyObject *info = PyList_New(0);
		return info;
	}
	
	Py_INCREF(Py_None);
	return Py_None;
}
%}

PyObject *__info();

