...
if sys.maxsize > 2 ** 32:
    ...
    class TestVideoSubscriberAccountErrors (TestCase):
        def testConstants(self):
            ...
            self.assertIsInstance(VideoSubscriberAccount.VSErrorInfoKeySAMLResponse, unicode)
            self.assertIsInstance(VideoSubscriberAccount.VSErrorInfoKeySAMLResponseStatus, unicode)
            self.assertIsInstance(VideoSubscriberAccount.VSErrorInfoKeyAccountProviderResponse, unicode)
            self.assertIsInstance(VideoSubscriberAccount.VSErrorInfoKeyUnsupportedProviderIdentifier, unicode)
            ...
            self.assertEqual(VideoSubscriberAccount.VSErrorCodeServiceTemporarilyUnavailable, 3)
