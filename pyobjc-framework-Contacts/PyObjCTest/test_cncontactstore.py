...
if sys.maxsize > 2**32:
    ...
    class TestCNContactStore (TestCase):
        def test_methods(self):
            ...
            self.assertResultIsBOOL(Contacts.CNContactStore.enumerateContactsWithFetchRequest_error_usingBlock_)
            self.assertArgIsOut(Contacts.CNContactStore.enumerateContactsWithFetchRequest_error_usingBlock_, 1)
            self.assertArgIsBlock(Contacts.CNContactStore.enumerateContactsWithFetchRequest_error_usingBlock_, 2, b'v@o^Z')
        ...
        @min_os_level('10.15')
        def test_methods(self):
            self.assertArgIsOut(Contacts.CNContactStore.enumeratorForContactFetchRequest_error_, 1)
            self.assertArgIsOut(Contacts.CNContactStore.enumeratorForChangeHistoryFetchRequest_error_, 1)
