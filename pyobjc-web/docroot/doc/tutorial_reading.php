...
<tt class="literal"><span class="pre">NSCoding</span></tt> protocol (<tt class="literal"><span class="pre">initWithCoder:</span></tt> is similar to <tt class="literal"><span class="pre">__setstate__</span></tt> of
Python's <tt class="literal"><span class="pre">pickle</span></tt> protocol), and then each object is sent an
<tt class="literal"><span class="pre">awakeFromNib:</span></tt> message so that they may do any initialization that depends
on a fully restored object graph (<tt class="literal"><span class="pre">pickle</span></tt> does not have this functionality
built-in).</p>
...
<li>Initialization is not always done in <tt class="literal"><span class="pre">__init__</span></tt> or equivalent, because
the object graph may not be completely unarchived until the first
<tt class="literal"><span class="pre">awakeFromNib:</span></tt> is called.</li>
...
common to use your own subclass of <tt class="literal"><span class="pre">NSApplication</span></tt> for the main application
object.  <tt class="literal"><span class="pre">NSApplication</span></tt> objects have a helper called its <tt class="literal"><span class="pre">delegate</span></tt>.
...
interested in.</p>
<p>For example, the method <tt class="literal"><span class="pre">applicationShouldTerminate:</span></tt> of the delegate
is called just before the application quits, and it has a chance to return
<tt class="literal"><span class="pre">NO</span></tt> if it is not appropriate to quit just yet.</p>
</div>
...
<tt class="literal"><span class="pre">NSApplicationMain</span></tt> or <tt class="literal"><span class="pre">PyObjCTools.AppHelper.runEventLoop</span></tt>, this will be
the instance of <tt class="literal"><span class="pre">NSApplication</span></tt>.  Currency Converter is not a document-based
application, so the MVC trio for the conversion window are in here too.  These
are named <tt class="literal"><span class="pre">Converter</span></tt>, <tt class="literal"><span class="pre">Window</span></tt> and <tt class="literal"><span class="pre">ConverterController</span></tt> respectively.</p>
...
action will go to the <tt class="literal"><span class="pre">ConverterController.convert_</span></tt> method.</p>
...
<tt class="literal"><span class="pre">ConverterController.invertRate_</span></tt>.  The solution: go to the &quot;MainMenu.nib&quot; window and
