...
if sys.maxsize > 2 ** 32:
    ...
    class TestSKAction (TestCase):
        ...
        def testMethods(self):
            ...
            self.assertArgIsBOOL(SpriteKit.SKAction.rotateToAngle_duration_shortestUnitArc_, 2)
            self.assertArgIsBOOL(SpriteKit.SKAction.animateWithTextures_timePerFrame_resize_restore_, 2)
            self.assertArgIsBOOL(SpriteKit.SKAction.animateWithTextures_timePerFrame_resize_restore_, 3)
            self.assertArgIsBOOL(SpriteKit.SKAction.playSoundFileNamed_waitForCompletion_, 1)
            self.assertArgIsBOOL(SpriteKit.SKAction.followPath_asOffset_orientToPath_duration_, 1)
            self.assertArgIsBOOL(SpriteKit.SKAction.followPath_asOffset_orientToPath_duration_, 2)
            self.assertArgIsSEL(SpriteKit.SKAction.performSelector_onTarget_, 0, b'v@:')

            self.assertArgIsBlock(SpriteKit.SKAction.customActionWithDuration_actionBlock_, 1, b"v@" + objc._C_CGFloat)
        ...
        def testMethods10_10(self):
            self.assertArgIsBOOL(SpriteKit.SKAction.followPath_asOffset_orientToPath_speed_, 1)
            self.assertArgIsBOOL(SpriteKit.SKAction.followPath_asOffset_orientToPath_speed_, 2)
