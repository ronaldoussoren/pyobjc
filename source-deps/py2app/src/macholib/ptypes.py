...
class MetaPackable(type):
    ...
    """
    def from_mmap(cls, mm, ptr):
        return cls.from_str(mm[ptr:ptr+cls._size_])

    def from_fileobj(cls, f):
        return cls.from_str(f.read(cls._size_))

    def from_str(cls, s):
        return cls.from_tuple(struct.unpack(cls._endian_ + cls._format_, s))
...
class Packable(BasePackable):
    ...
    def to_str(self):
        cls = type(self)
        return struct.pack(cls._endian_ + cls._format_, self)
...
class MetaStructure(MetaPackable):
    ...
    def from_tuple(cls, tpl):
        ...
        for begin, length, typ in cls._structmarks_:
            ...
            values.append(typ.from_tuple(tpl[begin:current]))
...
def test_ptypes():
    ...
    for fn, args in [
        ...
        myStructure = getattr(MyStructure, fn)(*args)
    ...
    for fn, args in [
        ...
        myFunStructure = getattr(MyFunStructure, fn)(*args)
    ...
    assert MyFunStructure.from_mmap(mm, 0) == myFunStructure
    ...
    assert MyFunStructure.from_mmap(mm, sizeof(MyFunStructure)) == myFunStructure
