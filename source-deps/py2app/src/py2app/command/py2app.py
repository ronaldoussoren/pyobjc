...
import shlex
import inspect
from itertools import imap, chain, ifilter, ifilterfalse
...
from py2app.util import fancy_split, byte_compile, make_loader, imp_find_module, copy_tree, move, fsencoding, strip_path
...
def hasFilenameFilter(n):
    return getattr(n, 'filename', None) is not None

#def notScriptFilter(n):
#    return not isinstance(n, modulegraph.Script)
...
class py2app(Command):
    ...
    def finalize_options (self):
        ...
        self.frameworks = fancy_split(self.frameworks)
        if not self.plist:
            self.plist = {}
    ...
    def get_runtime_preferences(self):
        ...
        fmwk = os.path.split(os.path.split(prefix)[0])[0]
        info = macholib.infoForFramework(fmwk)
        if info:
            runtime = info[1]
            info = macholib.infoForFramework(os.path.join(prefix, runtime))
            yield (
                '@executable_path/../Frameworks/%(name)s.framework/Versions/%(version)s/%(name)s' %
                dict(name=info[1], version=info[2])
            )
        else:
            runtime = 'libpython%s.dylib' % (sys.version[:3],)
            yield '@executable_path/../Frameworks/' + runtime
            runtime = os.path.join('lib', runtime)
        if self.semi_standalone or self.alias:
            yield os.path.join(prefix, runtime)
    ...
    def _run(self):
        ...
        filters = [hasFilenameFilter] + list(self.filters)
        flatpackages = {}
        while True:
    ...
    def find_dylibs(self, extensions):
        mm = macholib.MachMaster()
        for fn in extensions:
            mm.addFile(fn.filename)
    ...
    def create_binaries(self, py_files, pkgdirs, extensions, mm):
        ...
        print "*** copy extensions and dylibs ***"
        for filetype, fobj in mm.walkForCopy():
            if not self.semi_standalone and os.path.realpath(fobj.filename) == os.path.realpath(sys.executable):
                # skip including Python interpreter
                continue
            if filetype == 'bundle':
                # this is an extension
                ext = extmap.get(fobj.filename)
                dst = os.path.join(self.ext_dir,
                    os.path.dirname(ext.identifier.replace('.', os.sep)))
            else:
                # framework or dylib
                dst = self.framework_dir
            self.mkpath(dst)
            copier = self.copy_versioned_framework(fobj.copyname, fobj.basename)
            # XXX - hack hack hack
            if os.path.basename(fobj.copyname) == 'Python.framework':
                # do the copying now
                copier = lambda *args,**kwargs: None
                self.copy_python_framework(fobj.basename, fobj.copyname, dst)
            fobj.copyToFolder(dst,
                copyfile=self.copy_file, copytree=copier)
    ...
    def copy_versioned_framework(self, copyname, basename):
        ...
        info = macholib.infoForFramework(basename)
        if info is None or not info[2]:
            return self.copy_framework
        version = info[2]
        vsplit = os.path.join(os.path.realpath(copyname), 'Versions').split(os.sep)
        def condition(src, vsplit=vsplit, version=version):
            srcsplit = os.path.realpath(src).split(os.sep)
            if (
        ...
        def _copy_versioned_framework(infile, outfile, symlinks=False, condition=condition):
            self.copy_tree(infile, outfile, preserve_symlinks=symlinks, condition=condition)
        return _copy_versioned_framework
    
    def copy_framework(self, infile, outfile, symlinks=False):
        self.copy_tree(infile, outfile, preserve_symlinks=symlinks)
    
    def copy_python_framework(self, basename, infile, outfile):
        ...
        path, name, version = macholib.infoForFramework(basename)
        if version:
            indir = os.path.join(os.path.dirname(infile), os.path.dirname(basename))
            outdir = os.path.join(outfile, os.path.dirname(basename))
        else:
            indir = infile
            outdir = outfile
        ...
        for fn in [name, 'Resources/Info.plist', 'Resources/version.plist']:
            self.copy_file(
