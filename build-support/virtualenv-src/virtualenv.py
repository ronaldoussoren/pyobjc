...
virtualenv_version = "1.4.6.post1"
...
def make_exe(fn):
    if hasattr(os, 'chmod'):
        oldmode = os.stat(fn).st_mode & int('07777', 8)
...
def main():
    ...
    if not args:
        ...
        sys.exit(2)
    if len(args) > 1:
        print ('There must be only one argument: DEST_DIR (you gave ' + (
            ' '.join(args)) + ')')
        parser.print_help()
        sys.exit(2)
...
def call_subprocess(cmd, show_stdout=True,
    ...
    try:
        ...
        logger.fatal(
            "Error %s while executing command %s" % (sys.exc_info()[1], cmd_desc))
        raise
    all_output = []
    ...
    if sys.platform == 'win32':
        ...
        if ' ' in home_dir:
            try:
                ...
                print ('Error: the path "' + home_dir + '%s" has a space in it')
...
def install_python(home_dir, lib_dir, inc_dir, bin_dir, site_packages, clear):
    ...
    if sys.platform == 'darwin' and macosx_framework in prefix:
        ...
        if (sys.version_info[0] == 2 and sys.version_info[1] >= 7) or \
            ...
            shutil.copy(os.path.join(prefix, 'bin/python'), py_executable)
