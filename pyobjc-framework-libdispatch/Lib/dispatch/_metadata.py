# This file is generated by objective.metadata
#
# flake8: noqa

import objc, sys
from typing import NewType

if sys.maxsize > 2**32:

    def sel32or64(a, b):
        return b

else:

    def sel32or64(a, b):
        return a


if objc.arch == "arm64":

    def selAorI(a, b):
        return a

else:

    def selAorI(a, b):
        return b


misc = {}
misc.update(
    {
        "timespec": objc.createStructType(
            "dispatch.timespec", b"{timespec=qq}", ["tv_sec", "tv_nsec"]
        )
    }
)
constants = """$$"""
enums = """$DISPATCH_AUTORELEASE_FREQUENCY_INHERIT@0$DISPATCH_AUTORELEASE_FREQUENCY_NEVER@2$DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM@1$DISPATCH_BLOCK_ASSIGN_CURRENT@4$DISPATCH_BLOCK_BARRIER@1$DISPATCH_BLOCK_DETACHED@2$DISPATCH_BLOCK_ENFORCE_QOS_CLASS@32$DISPATCH_BLOCK_INHERIT_QOS_CLASS@16$DISPATCH_BLOCK_NO_QOS_CLASS@8$DISPATCH_IO_RANDOM@1$DISPATCH_IO_STOP@1$DISPATCH_IO_STREAM@0$DISPATCH_IO_STRICT_INTERVAL@1$DISPATCH_MACH_SEND_DEAD@1$DISPATCH_MEMORYPRESSURE_CRITICAL@4$DISPATCH_MEMORYPRESSURE_NORMAL@1$DISPATCH_MEMORYPRESSURE_WARN@2$DISPATCH_ONCE_INLINE_FASTPATH@1$DISPATCH_PROC_EXEC@536870912$DISPATCH_PROC_EXIT@2147483648$DISPATCH_PROC_FORK@1073741824$DISPATCH_PROC_SIGNAL@134217728$DISPATCH_QUEUE_PRIORITY_BACKGROUND@-32768$DISPATCH_QUEUE_PRIORITY_DEFAULT@0$DISPATCH_QUEUE_PRIORITY_HIGH@2$DISPATCH_QUEUE_PRIORITY_LOW@-2$DISPATCH_TIMER_STRICT@1$DISPATCH_TIME_FOREVER@18446744073709551615$DISPATCH_TIME_NOW@0$DISPATCH_VNODE_ATTRIB@8$DISPATCH_VNODE_DELETE@1$DISPATCH_VNODE_EXTEND@4$DISPATCH_VNODE_FUNLOCK@256$DISPATCH_VNODE_LINK@16$DISPATCH_VNODE_RENAME@32$DISPATCH_VNODE_REVOKE@64$DISPATCH_VNODE_WRITE@2$DISPATCH_WALLTIME_NOW@18446744073709551614$NSEC_PER_MSEC@1000000$NSEC_PER_SEC@1000000000$NSEC_PER_USEC@1000$USEC_PER_SEC@1000000$"""
misc.update(
    {
        "dispatch_block_flags_t": NewType("dispatch_block_flags_t", int),
        "dispatch_autorelease_frequency_t": NewType(
            "dispatch_autorelease_frequency_t", int
        ),
    }
)
misc.update({})
misc.update(
    {
        "DISPATCH_QUEUE_SERIAL": None,
        "DISPATCH_TARGET_QUEUE_DEFAULT": None,
        "DISPATCH_CURRENT_QUEUE_LABEL": None,
        "DISPATCH_DATA_DESTRUCTOR_DEFAULT": None,
    }
)
functions = {
    "dispatch_io_create_with_io": (
        b"@Q@@@?",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"i"}},
                    }
                }
            },
        },
    ),
    "dispatch_source_get_data": (b"Q@",),
    "dispatch_data_create_concat": (b"@@@", "", {"retval": {"already_retained": True}}),
    "dispatch_semaphore_create": (b"@q", "", {"retval": {"already_retained": True}}),
    "dispatch_activate": (b"v@",),
    "dispatch_io_set_interval": (b"v@QQ",),
    "dispatch_assert_queue_not": (b"v@",),
    "dispatch_barrier_async": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_after": (
        b"vQ@@?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "os_workgroup_create_with_workgroup": (
        b"@^t@",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
    "os_workgroup_get_working_arena": (
        b"^v@^I",
        "",
        {
            "retval": {"c_array_of_variable_length": True},
            "arguments": {1: {"type_modifier": "o"}},
        },
    ),
    "dispatch_io_get_descriptor": (b"i@",),
    "dispatch_source_set_timer": (b"v@QQQ",),
    "dispatch_get_current_queue": (b"@",),
    "os_workgroup_set_working_arena": (
        b"i@^vI@?",
        "",
        {
            "arguments": {
                1: {"type_modifier": "n", "c_array_of_variable_length": True},
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {
                                "type": b"^v",
                                "type_modifier": "n",
                                "c_array_of_variable_length": True,
                            },
                        },
                    }
                },
            }
        },
    ),
    "dispatch_io_close": (b"v@Q",),
    "dispatch_source_get_mask": (b"Q@",),
    "dispatch_queue_attr_make_initially_inactive": (b"@@",),
    "dispatch_block_create": (
        b"@?Q@?",
        "",
        {
            "retval": {
                "callable": {"retval": {"type": "v"}, "arguments": {0: {"type": "^v"}}},
                "already_retained": True,
            },
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            },
        },
    ),
    "dispatch_block_perform": (
        b"vQ@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_allow_send_signals": (b"ii",),
    "os_workgroup_testcancel": (b"B@",),
    "dispatch_block_testcancel": (
        b"q@?",
        "",
        {
            "arguments": {
                0: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_group_async": (
        b"v@@@?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_read": (
        b"viQ@@?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"comment": "dispatch_data_t", "type": b"@"},
                            2: {"type": b"i"},
                        },
                    }
                }
            }
        },
    ),
    "dispatch_group_leave": (b"v@",),
    "dispatch_assert_queue_barrier": (b"v@",),
    "dispatch_once": (
        b"v^q@?",
        "",
        {
            "arguments": {
                0: {"c_array_of_fixed_length": 1, "type_modifier": "N"},
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                },
            }
        },
    ),
    "dispatch_source_set_registration_handler": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_once_f": (
        b"v^q^v^?",
        "",
        {
            "arguments": {
                0: {"c_array_of_fixed_length": 1, "type_modifier": "N"},
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": False,
                },
            }
        },
    ),
    "dispatch_async_and_wait_f": (
        b"v@^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_get_context": (b"^v@",),
    "dispatch_set_target_queue": (b"v@@",),
    "os_workgroup_copy_port": (b"i@^I", "", {"arguments": {1: {"type_modifier": "o"}}}),
    "dispatch_data_copy_region": (
        b"@@Q^Q",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {2: {"type_modifier": "o"}},
        },
    ),
    "dispatch_async_and_wait": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_workloop_set_autorelease_frequency": (b"v@Q",),
    "dispatch_queue_set_specific": (
        b"v@^v^v^?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_io_write": (
        b"v@q@@@?",
        "",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"B"},
                            2: {"type": b"@"},
                            3: {"type": b"i"},
                        },
                    }
                }
            }
        },
    ),
    "os_workgroup_max_parallel_threads": (b"i@^{os_workgroup_mpt_attr_s=}",),
    "dispatch_barrier_async_f": (
        b"v@^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_block_cancel": (
        b"v@?",
        "",
        {
            "arguments": {
                0: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_group_async_f": (
        b"v@@^v^?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "_dispatch_once_f": (
        b"v^q^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_apply_f": (
        b"vQ@^v^?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"Q"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_set_context": (b"v@^v",),
    "dispatch_queue_attr_make_with_qos_class": (b"@@Ii",),
    "dispatch_suspend": (b"v@",),
    "dispatch_set_finalizer_f": (
        b"v@^?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "os_workgroup_interval_update": (b"i@Q@",),
    "dispatch_sync": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_block_notify": (
        b"v@?@@?",
        "",
        {
            "arguments": {
                0: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                },
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                },
            }
        },
    ),
    "dispatch_queue_create": (
        b"@^t@",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
    "dispatch_group_wait": (b"q@Q",),
    "dispatch_main": (b"v",),
    "dispatch_barrier_sync": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_source_set_cancel_handler_f": (
        b"v@^?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_cancel": (b"v@",),
    "dispatch_source_set_event_handler_f": (
        b"v@^?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_queue_get_specific": (b"^v@^v",),
    "dispatch_async_f": (
        b"v@^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "os_workgroup_leave": (b"v@^{os_workgroup_join_token_opaque_s=}",),
    "os_workgroup_join": (b"i@^{os_workgroup_join_token_opaque_s=}",),
    "dispatch_source_set_event_handler": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_time": (b"QQq",),
    "dispatch_queue_get_label": (
        b"^t@",
        "",
        {"retval": {"c_array_delimited_by_null": True}},
    ),
    "dispatch_source_cancel": (b"v@",),
    "dispatch_group_enter": (b"v@",),
    "dispatch_io_create_with_path": (
        b"@Q^tiS@@?",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {
                1: {"c_array_delimited_by_null": True, "type_modifier": "n"},
                5: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"i"}},
                    }
                },
            },
        },
    ),
    "dispatch_testcancel": (b"l@",),
    "dispatch_io_read": (
        b"v@qQ@@?",
        "",
        {
            "arguments": {
                4: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"B"},
                            2: {"type": b"@"},
                            3: {"type": b"i"},
                        },
                    }
                }
            }
        },
    ),
    "dispatch_source_testcancel": (b"q@",),
    "dispatch_source_set_registration_handler_f": (
        b"v@^?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "os_workgroup_cancel": (b"v@",),
    "dispatch_barrier_sync_f": (
        b"v@^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": False,
                }
            }
        },
    ),
    "dispatch_group_notify_f": (
        b"v@@^v^?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "os_workgroup_create_with_port": (
        b"@^tI",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
    "dispatch_data_create_subrange": (
        b"@@QQ",
        "",
        {"retval": {"already_retained": True}},
    ),
    "os_workgroup_interval_start": (b"i@QQ@",),
    "dispatch_semaphore_signal": (b"q@",),
    "dispatch_group_create": (b"@", "", {"retval": {"already_retained": True}}),
    "dispatch_barrier_async_and_wait": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_queue_get_qos_class": (
        b"I@^i",
        "",
        {"arguments": {1: {"type_modifier": "o"}}},
    ),
    "dispatch_sync_f": (
        b"v@^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": False,
                }
            }
        },
    ),
    "dispatch_workloop_set_os_workgroup": (b"v@@",),
    "dispatch_get_main_queue": (b"@",),
    "dispatch_data_apply": (
        b"B@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"B"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"type": b"@"},
                            2: {"type": b"L"},
                            3: {"type": b"n^v", "c_array_length_in_arg": 4},
                            4: {"type": b"L"},
                        },
                    }
                }
            }
        },
    ),
    "dispatch_write": (
        b"vi@@@?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {
                            0: {"type": b"^v"},
                            1: {"comment": "dispatch_data_t", "type": b"@"},
                            2: {"type": b"i"},
                        },
                    }
                }
            }
        },
    ),
    "dispatch_io_barrier": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_get_global_queue": (b"@qQ",),
    "dispatch_barrier_async_and_wait_f": (
        b"v@^v^?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_walltime": (
        b"Q^{timespec=qq}q",
        "",
        {"arguments": {0: {"type_modifier": "n"}}},
    ),
    "dispatch_assert_queue": (b"v@",),
    "dispatch_get_specific": (b"^v^v",),
    "dispatch_source_get_handle": (b"Q@",),
    "dispatch_data_create": (
        b"@^vQ@@?",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {
                0: {"c_array_length_in_arg": 1, "type_modifier": "n"},
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                },
            },
        },
    ),
    "dispatch_resume": (b"v@",),
    "dispatch_semaphore_wait": (b"q@Q",),
    "dispatch_io_create": (
        b"@Qi@@?",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"i"}},
                    }
                }
            },
        },
    ),
    "dispatch_source_create": (
        b"@^{dispatch_source_type_s=}QQ@",
        "",
        {"retval": {"already_retained": True}},
    ),
    "dispatch_wait": (b"q^vQ",),
    "dispatch_io_set_high_water": (b"v@Q",),
    "dispatch_workloop_create": (
        b"@^t",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
    "dispatch_workloop_create_inactive": (
        b"@^t",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
    "_dispatch_once": (b"v^q@?",),
    "dispatch_data_create_map": (
        b"@@^^v^Q",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {
                1: {"c_array_length_in_arg": 2, "type_modifier": "o"},
                2: {"type_modifier": "o"},
            },
        },
    ),
    "os_workgroup_interval_finish": (b"i@@",),
    "dispatch_async": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_group_notify": (
        b"v@@@?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_block_create_with_qos_class": (
        b"@?QIi@?",
        "",
        {
            "retval": {
                "callable": {"retval": {"type": "v"}, "arguments": {0: {"type": "^v"}}},
                "already_retained": True,
            },
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            },
        },
    ),
    "dispatch_io_set_low_water": (b"v@Q",),
    "dispatch_data_get_size": (b"Q@",),
    "dispatch_block_wait": (
        b"q@?Q",
        "",
        {
            "arguments": {
                0: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "dispatch_after_f": (
        b"vQ@^v^?",
        "",
        {
            "arguments": {
                3: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    },
                    "callable_retained": True,
                }
            }
        },
    ),
    "dispatch_set_qos_class_floor": (b"v@Ii",),
    "dispatch_apply": (
        b"vQ@@?",
        "",
        {
            "arguments": {
                2: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}, 1: {"type": b"L"}},
                    }
                }
            }
        },
    ),
    "dispatch_source_merge_data": (b"v@Q",),
    "dispatch_queue_attr_make_with_autorelease_frequency": (b"@@Q",),
    "os_workgroup_parallel_create": (
        b"@^t@",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
    "dispatch_source_set_cancel_handler": (
        b"v@@?",
        "",
        {
            "arguments": {
                1: {
                    "callable": {
                        "retval": {"type": b"v"},
                        "arguments": {0: {"type": b"^v"}},
                    }
                }
            }
        },
    ),
    "_dispatch_object_validate": (b"v@",),
    "dispatch_queue_create_with_target": (
        b"@^t@@",
        "",
        {
            "retval": {"already_retained": True},
            "arguments": {0: {"c_array_delimited_by_null": True, "type_modifier": "n"}},
        },
    ),
}
aliases = {
    "DISPATCH_APPLY_QUEUE_ARG_NULLABILITY": "_Nullable",
    "DISPATCH_QUEUE_PRIORITY_BACKGROUND": "INT16_MIN",
    "dispatch_once": "_dispatch_once",
    "DISPATCH_RETURNS_RETAINED": "OS_OBJECT_RETURNS_RETAINED",
    "dispatch_once_f": "_dispatch_once_f",
}
misc.update(
    {
        "os_workgroup_join_token_opaque_t": objc.createOpaquePointerType(
            "os_workgroup_join_token_opaque_t", b"^{os_workgroup_join_token_opaque_s=}"
        ),
        "os_workgroup_max_parallel_threads_attr_t": objc.createOpaquePointerType(
            "os_workgroup_max_parallel_threads_attr_t",
            b"^{os_workgroup_max_parallel_threads_attr_s=}",
        ),
    }
)
expressions = {
    "DISPATCH_CURRENT_QUEUE_LABEL": "None",
    "DISPATCH_QUEUE_CONCURRENT_WITH_AUTORELEASE_POOL": "dispatch_queue_attr_make_with_autorelease_frequency(DISPATCH_QUEUE_CONCURRENT, DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM)",
    "DISPATCH_QUEUE_SERIAL": "None",
    "DISPATCH_QUEUE_SERIAL_WITH_AUTORELEASE_POOL": "dispatch_queue_attr_make_with_autorelease_frequency(DISPATCH_QUEUE_SERIAL, DISPATCH_AUTORELEASE_FREQUENCY_WORK_ITEM)",
    "DISPATCH_APPLY_AUTO": "None",
    "DISPATCH_QUEUE_SERIAL_INACTIVE": "dispatch_queue_attr_make_initially_inactive(DISPATCH_QUEUE_SERIAL)",
    "DISPATCH_DATA_DESTRUCTOR_DEFAULT": "None",
    "DISPATCH_QUEUE_CONCURRENT_INACTIVE": "dispatch_queue_attr_make_initially_inactive(DISPATCH_QUEUE_CONCURRENT)",
    "DISPATCH_TIME_NOW": "(0ull)",
    "DISPATCH_TARGET_QUEUE_DEFAULT": "None",
}
formal_protocols = {
    "OS_dispatch_data": "OS_dispatch_data",
    "OS_dispatch_group": "OS_dispatch_group",
    "OS_dispatch_io": "OS_dispatch_io",
    "OS_dispatch_object": "OS_dispatch_object",
    "OS_dispatch_queue": "OS_dispatch_queue",
    "OS_dispatch_queue_attr": "OS_dispatch_queue_attr",
    "OS_dispatch_queue_concurrent": "OS_dispatch_queue_concurrent",
    "OS_dispatch_queue_global": "OS_dispatch_queue_global",
    "OS_dispatch_queue_main": "OS_dispatch_queue_main",
    "OS_dispatch_queue_serial": "OS_dispatch_queue_serial",
    "OS_dispatch_queue_serial_executor": "OS_dispatch_queue_serial_executor",
    "OS_dispatch_semaphore": "OS_dispatch_semaphore",
    "OS_dispatch_source": "OS_dispatch_source",
    "OS_dispatch_workloop": "OS_dispatch_workloop",
}

# END OF FILE
